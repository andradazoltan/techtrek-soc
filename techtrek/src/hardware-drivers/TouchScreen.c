...
void InitTouch(void) {
  ...
  TouchScreen_DivisorLatchLSB =
      0x45; // Set Divisor latch (LSB and MSB) to get required baud rate
  TouchScreen_DivisorLatchMSB = 0x01;
  TouchScreen_LineControlReg =
      0x03; // Configure other settings: 8 bit data, 1 stop bit, no parity, etc.
  TouchScreen_FifoControlReg =
      TouchScreen_FifoControlReg | 0x06; // Reset the Fifo's
  TouchScreen_FifoControlReg =
      TouchScreen_FifoControlReg ^
      0x06; // Clear all bits in the FiFo control registers
...
static int putcharTS(int c) {
  ...
  TouchScreen_TransmitterFifo =
      (char)c; // Write character to Transmitter FIFO register
  return c;
}
...
static int getcharTS(void) {
  ...
  return (int)
      TouchScreen_ReceiverFifo; // Read new character from ReceiverFiFo register
}
