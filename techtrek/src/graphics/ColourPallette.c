...
//
// Constants for each pallette number using the name for the colour can be found
// in the header file "Colours.h"
//
// this data represents the 24 bit RGB values of 256 colours. DE2 can display
// all 256 simultaneously, but DE1 can only display 64 at any one time. It
// should be setup in hardware in a ROM file in Quartus but the software ('C'
// code) version of the data is also given below and can be used as a reference
// to the original data in graphics chip ROM
//
// You should program the colour pallette (DE2 = 256, DE1 = 64) at the start,
// i.e in main() by calling ProgramPallette(BLACK, ColourPalletteData[0]) for
// each colour and programming the colour(s) you want, 1 RGB value per pallette.
// BLACK is enumerated as 0 in Colours.h header file
//
// You should include this as part of the project so it gets compiled, but if
// you want to reference it in multiple source file you should put an "extern"
// declaration in those source files e.g. extern const unsigned int
// ColourPalletteData[256]
