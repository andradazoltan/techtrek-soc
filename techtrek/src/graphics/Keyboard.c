...
#define MAX_KEYS_PER_ROW                                                       \
  10 // Maximum number of keys in a keyboard row (top row)
#define KEY_X_SIZE                                                             \
  ((XRES - 50) /                                                               \
   MAX_KEYS_PER_ROW) // Number of pixels a key can take up horizontally
#define KEY_Y_SIZE                                                             \
  ((YRES - 40) / (2 * 3)) // Number of pixels a key can take up vertically
...
keys_t keys[] = {{.keyText = "QWERTYUIOP",
  ...
  for (int row = 0; row < 3; row++) {
    ...
    for (int key = 0; curr_row.keyText[key] != '\0'; key++) {
      ...
      if (curr_row.keyText[key] == 'x') { // Backspace key
        ...
        OutGraphicsCharFont3(x1 + KEY_X_SIZE / 2 - 15, y1 + KEY_Y_SIZE / 2 - 15,
                             WHITE, MAROON, to_print, 0);
        ...
        if (firstSpace) {
          ...
          OutGraphicsCharFont3(x1 - KEY_X_SIZE / 2 + 5,
                               y1 + KEY_Y_SIZE / 2 - 15, WHITE, MAROON,
                               to_print, 0);
        }
        ...
        OutGraphicsCharFont3(x1 + KEY_X_SIZE / 2 - 10, y1 + KEY_Y_SIZE / 2 - 15,
                             WHITE, MAROON, to_print, 0);
      }
    }
  }
...
/*
 * Called when a touch screen release has been detected while on the
 * HAZARD_SCREEN. This function will determine which key was released, and print
 * it to the text box while updating the cursor at the same time.
 */
void keyRelease(int x, int y) {
  keys_t row;

  // The recorded row Y-coordinates are relative to the starting position where
  // the keyboard was drawn
  int yMod = y - keyboardYStart;

  // First find the row that was pressed
  if ((yMod >= keys[0].topLeftYCoord) && (yMod <= keys[0].bottomRightYCoord)) {
    row = keys[0];
  } else if ((yMod >= keys[1].topLeftYCoord) &&
             (yMod <= keys[1].bottomRightYCoord)) {
    row = keys[1];
  } else if ((yMod >= keys[2].topLeftYCoord) &&
             (yMod <= keys[2].bottomRightYCoord)) {
    row = keys[2];
  } else
    return;
  ...
  if (to_print[0] == 'x') { // Backspace key
    if ((cursorXPos - FONT3_XPIXELS) >
        tbXStart) { // Don't let it erase beyond bounds of box
      cursorXPos -= FONT3_XPIXELS;
      OutGraphicsCharFont3(cursorXPos, cursorYPos + 10, WHITE, BLACK, to_print,
                           1);
    ...
    OutGraphicsCharFont3(tbXStart + 10, cursorYPos + 10, WHITE, BLACK,
                         "Thank you for reporting.", 0);
    ...
    if ((cursorXPos + FONT3_XPIXELS) <
        tbXEnd) { // Don't let it print outside the box
      OutGraphicsCharFont3(cursorXPos, cursorYPos + 10, WHITE, BLACK, to_print,
                           0);
      cursorXPos += FONT3_XPIXELS;
