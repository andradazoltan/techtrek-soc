...
/*
 * Draws an image on screen given the starting coordinates
    ...
    for (int cur_byte = 0; cur_byte < valid_bytes; cur_byte++) {
      for (int bit = 0; bit < 8; bit++) {
        ...
        if (x_val >= (*myBMP).header.width) {
          ...
          if (pixel_val > 0 && x_val + topLeftX >= 0 &&
              x_val + topLeftX < XRES && y_val + topLeftY >= 0 &&
              y_val + topLeftY < YRES) {
...
/*
 * Frees the memory associated with the image data inside a BMP struct.
 *
 * Returns: 0 if data is NULL or previously freed, otherwise returns 1.
 */
