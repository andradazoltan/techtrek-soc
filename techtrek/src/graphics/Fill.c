...
void Fill(int _x, int _y, int _FillColour, int _BoundaryColour) {
  ...
  while (!IsStackEmpty()) // keep going until no more items on the stack
  {
    ...
    SaveX = x++; // save the x coord of the the point we just filled and move
                 // one pixel right

    while ((char)(ReadAPixel(x, y)) !=
           (char)(BoundaryColour)) // if new pixel is not the boundary colour
      WriteAPixel(
          x++, y,
          FillColour); // fill it and keep moving right along a horizontal line

    // must have found the boundary colour when moving right
    XRight = x - 1; // save X coord of the last filled pixel on this line when
                    // moving right
    ...
    while ((char)(ReadAPixel(x, y)) !=
      ...
      WriteAPixel(
          x--, y,
          FillColour); // fill it and keep moving left along a horizontal line

    XLeft = x + 1; // save X coord of the last filled pixel on this line when
                   // moving left
    ...
    // until we find something that is neither filled nor boundary colour as it
    // represents something on the line that may be a pixel to fill

    do {
      PixelColour = ReadAPixel(x++, y);
    } while (((char)(PixelColour) == (char)(BoundaryColour)) ||
             ((char)(PixelColour) == (char)(FillColour)));

    x--;

    // to get here we must have found something that needs filling i.e. the
    // above loop found that the line below was not a complete boundary edge or
    // filled if we are still less than the previous right most X coord then it
    // would be a new point that we need to seed
    while (x <= XRight) {
      // seed the scan line below
      // if the pixel at x,y is not a boundary colour and less than extreme
      // right

      // skip over any pixels already filled
      while (((char)(ReadAPixel(x, y)) != (char)(BoundaryColour)) &&
             (x <= XRight))
        ++x;
      ...
      // continue checking in case the span is interrupted by another shape
      // inside the one we are trying to fill

      ++x;

      // skip over anything that is filled or boundary (i.e. other shape) inside
      // the one we are trying to fill
      do {
        PixelColour = ReadAPixel(x++, y);
      } while (((char)(PixelColour) == (char)(BoundaryColour)) ||
               ((char)(PixelColour) == (char)(FillColour)));
