 ...
 * Main Screen Objects
 */
object_t mainScreen[] = {{// Info Button
                          .type = RECT,
                          .colour = CHOCOLATE,
                          .text = "INFO",
                          .textXCoord = 630,
                          .func = &drawInfoScreen,
                          .rect = {.topLeftXCoord = 565,
                                   .topLeftYCoord = 15,
...
object_t helpScreen[] = {{//  Button
                          .type = RECT,
                          .colour = MAROON,
                          .text = "Unconscious",
                          .textXCoord = 90,
                          .func = (void *)&lua_postHelp,
                          .rect = {.topLeftXCoord = 70,
                                   .topLeftYCoord = 150,
...
object_t mapScreen[] = {{// Back Button
                         .type = RECT,
                         .colour = CADET_BLUE,
                         .text = "<- BACK",
                         .textXCoord = 660,
                         .func = &drawMainScreen,
                         .rect = {.topLeftXCoord = 580,
                                  .topLeftYCoord = 0,
...
object_t infoScreen[] = {
    ...
    {// Enter Button
     ...
     .textXCoord = 645,
     .func = &sendRating,
     .rect = {.topLeftXCoord = 620,
              .topLeftYCoord = 410,
              .bottomRightXCoord = 760,
              .bottomRightYCoord = 450}},
};

object_t warningsScreen[] = {{// Back Button
                              .type = RECT,
                              .colour = RED,
                              .text = "<- BACK",
                              .textXCoord = 660,
  ...
  for (int i = 0; i < numObjs; i++) {
    ...
    if (temp.type == RECT) {
      FillRect(temp.rect.topLeftXCoord, temp.rect.topLeftYCoord,
               temp.rect.bottomRightXCoord, temp.rect.bottomRightYCoord,
               temp.colour);

      int yCoord = (temp.rect.topLeftYCoord + temp.rect.bottomRightYCoord) / 2 -
                   (FONT3_YPIXELS / 2);
      OutGraphicsCharFont3(temp.textXCoord, yCoord, WHITE, temp.colour,
                           temp.text, 0);
    } else if (temp.type == CIRCLE) {
      // Don't use FillCircle() unless you want a trippy concentric circle
      Circle(temp.circle.centerXCoord, temp.circle.centerYCoord,
             temp.circle.radius, temp.colour);
      Fill(temp.circle.centerXCoord, temp.circle.centerYCoord, temp.colour,
           temp.colour);
    } else if (temp.type == GRAPH_HOR) {
      barGraphHor(temp.graph_hor.topLeftXCoord, temp.graph_hor.topLeftYCoord,
                  temp.graph_hor.bottomRightXCoord,
                  temp.graph_hor.bottomRightYCoord, temp.colour, BLACK,
                  *temp.graph_hor.percent);
    }
  }
}
...
void sendEmergency(int x, int y) {
  for (int i = 0; i < 6; i++) {
    if (IsObjectPressed(x, y, helpScreen[i])) {
      ...
      // Replace all spaces with a period since space is a special character in
      // a query
      for (int j = 0; j < 42; j++) {
        if (msg[j] == ' ')
          msg[j] = '.';
