...
HikerCam::HikerCam()
    : mSystem(NULL), mCamera(NULL), endAcquistionSignal(false) {
  ...
  try {
    ...
    while (!endAcquistionSignal) {
      ...
      if (img->IsIncomplete()) {
        ...
        InferenceBoundingBoxResult boundingBoxData =
            chunkData.GetInferenceBoundingBoxResult();
...
void HikerCam::EndAcquisition(void) { endAcquistionSignal.store(true); }
...
int HikerCam::EnableInference(INodeMap &nodeMap) {
  ...
  CFloatPtr boundingBoxThreshold =
      nodeMap.GetNode("InferenceBoundingBoxThreshold");
