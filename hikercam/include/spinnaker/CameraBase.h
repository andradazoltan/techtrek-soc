...
public:
   ...
   * the user to unregister events before the camera can be re-initialized
   * again.
   *
   * @see Init()
  ...
  /*
   * WritePort
   * Writes a remote port on a physical Camera. This function can be used to
   * write registers on Cameras.
   *
   * Caution: Only perform direct read/write to a register if the register isn't
   *          supported in the device nodemap. Otherwise the camera and nodemap
   *          may be left in an undefined state after the register read/write.
   ...
   * acquired using GetNextImage() need to be released first using
   * image->Release() before calling EndAcquisition().  All buffers in the input
   * pool and output queue will be discarded when EndAcquisition() is called.
   *
   * @see Init()
   * @see BeginAcquisition()
  ...
  /**
   * GetBufferOwnership
   * Gets data buffer ownership.  The data buffers can be owned by System or
   * User. If the system owns the buffers, the memory required for the buffers
   * are allocated and freed by the library.  If user owns the buffers, the user
   * is responsible for allocating and ultimately freeing the memory.  By
   * default, data buffers are owned by the library.
   *
  ...
  /**
   * SetBufferOwnership
   * Sets data buffer ownership.  The data buffers can be owned by System or
   * User. If the system owns the buffers, the memory required for the buffers
   * are allocated and freed by the library.  If user owns the buffers, the user
   * is responsible for allocating and ultimately freeing the memory.  By
   * default, data buffers are owned by the library.
   ...
   * @param totalSize The total size of the memory allocated for the user
   * buffers (in bytes)
   */
   ...
   * Each pointer to a buffer must have enough memory to hold one image. To
   * prevent image tearing when working with USB3 cameras, the size of each
   * buffer should be equal to:
   *     ((unsigned int) (bufferSize + 1024 - 1) / 1024) * 1024
   * where 1024 is the USB3 packet size.
   *
   ...
   * @see GetUserBufferTotalSize()
   *
   * @param ppMemBuffers Pointer to pointers that each point to a single user
   * memory buffer to be written to
   * @param bufferCount The number of user memory buffers
   * @param bufferSize The size of the memory allocated for each user buffer (in
   * bytes)
   */
  void SetUserBuffers(void **const ppMemBuffers, const uint64_t bufferCount,
   ...
   * support one stream so the default streamID is 0 but if a camera supports
   * multiple streams the user can input the streamID to select from which
   * stream to grab images
   *
   * @see Init()
   * @see BeginAcquisition()
   * @see EndAcquisition()
  ...
  /**
   * RegisterEvent(Event &)
   * Registers a specific event for the camera.  The camera has to be
   * initialized first with a call to Init() before registering for events.
   *
   * @see Init()
   *
   ...
   * Returns the largest packet size that can be safely used on the interface
