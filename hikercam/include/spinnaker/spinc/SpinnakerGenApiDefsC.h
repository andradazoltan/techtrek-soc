...
typedef enum _spinSign { Signed, Unsigned, _UndefinedSign } spinSign;
...
//! typedef describing the different validity checks which can be performed on
//! an XML file
...
typedef enum _spinLinkType {
  ctAllDependingNodes, //!> All nodes which will be invalidated if this node
                       //!becomes invalid
  ctAllTerminalNodes,  //!> All terminal nodes which may be written to by this
                       //!node
  ctInvalidators,    //!> List of references to nodes which may invalidate this
                     //!node
  ...
  ctDependingChildren //!> All child nodes which will cause this node to be
                      //!invalidated
} spinLinkType;
