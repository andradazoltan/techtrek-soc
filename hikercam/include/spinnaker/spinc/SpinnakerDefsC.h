 ...
 * events.
 */
typedef void (*spinDeviceEventFunction)(const spinDeviceEventData hEventData,
                                        const char *pEventName,
                                        void *pUserData);

typedef void (*spinImageEventFunction)(const spinImage hImage, void *pUserData);

typedef void (*spinArrivalEventFunction)(uint64_t deviceSerialNumber,
                                         void *pUserData);

typedef void (*spinRemovalEventFunction)(uint64_t deviceSerialNumber,
                                         void *pUserData);

typedef void (*spinLogEventFunction)(const spinLogEventData hEventData,
                                     void *pUserData);
...
typedef enum _actionCommandStatus {
  ...
  /* The device is not synchronized to a master clock to be used as time
     reference. Typically used when scheduled action commands cannot be
     scheduled for a future time since the reference time coming from IEEE 1588
     is not locked. */
  ACTION_COMMAND_STATUS_NO_REF_TIME = 0x8013,

  /* Returned when the scheduled action commands queue is full and the device
     cannot accept the additional request. */
  ACTION_COMMAND_STATUS_OVERFLOW = 0x8015,

  /* The requested scheduled action command was requested at a point in time
     that is in the past. */
  ACTION_COMMAND_STATUS_ACTION_LATE = 0x8016,

  /* Generic Error. Try enabling the Extended Status Code 2.0 bit on gvcp
     configuration register in order to receive more meaningful/detailed
     acknowledge messages from the device. */
