...
public:
  ...
  /**
   * Returns a pointer to a camera object at the "index". This function will
   * throw a Spinnaker exception with SPINNAKER_ERR_INVALID_PARAMETER error if
   ...
   * function will return a NULL CameraPtr if no matching camera serial is
   * found.
   *
   ...
   * interface. It is important that the camera list is destroyed or is cleared
   * before calling system->ReleaseInstance() or else the call to
   * system->ReleaseInstance() will result in an error message thrown that a
   ...
   * counted object. This function will throw a Spinnaker exception with
  ...
  /**
   * Removes a camera using its serial number and destroys its corresponding
   * reference counted object. This function will throw a Spinnaker exception
   * with SPINNAKER_ERR_NOT_AVAILABLE error if no matching camera serial is
   * found.
   *
