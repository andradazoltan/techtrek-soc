...
public:
   ...
   *
   * @param updateCameras A flag used to issue an updateCameras() call
   * internally before getting the camera list
   *
   * @return An CameraList object that contains a list of cameras on this
   * interface.
   */
  CameraList GetCameras(bool updateCameras = true) const;

  /**
   * Updates the list of cameras on this interface.  This function needs to be
   * called before any cameras can be discovered using GetCameras().
   * System::GetCameras() will automatically call this function for each
   * interface it enumerates.  If the list changed after the last time
   * System::GetCameras() or UpdateCameras() was called then the return value
   * will be true, otherwise it is false.
   *
   ...
   *
   */
  void SendActionCommand(unsigned int deviceKey, unsigned int groupKey,
