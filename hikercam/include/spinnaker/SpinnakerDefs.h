 ...
 */
const uint64_t EVENT_TIMEOUT_NONE =
    0; // Do not wait.  GetNextImage will return immediately.
const uint64_t EVENT_TIMEOUT_INFINITE =
    0xFFFFFFFFFFFFFFFF; // Never timeout.  GetNextImage will wait indefinitely.
...
enum DEPRECATED_CLASS(
    "This enum has been deprecated. Polarization images are now created "
    "through specific functions the ImageUtilityPolarization class.")
    PolarizationAlgorithm {
      /** No polarization. */
      NO_POLARIZATION,
      ...
      STOKES_S0_GRAYSCALE,
      /** Extracts a BGRa8 pixel format Heatmap representation of the Stokes'
         parameter image S0. */
      STOKES_S0_HEATMAP,
      ...
      /** Extracts a BGRa8 pixel format Heatmap representation of the Stokes'
         parameter image S2. */
      STOKES_S2_HEATMAP,
      /** Extracts a Mono8 pixel format image representation of the DoLP (Degree
         of Linear Polarization). */
      DOLP_GRAYSCALE,
      /** Extracts a BGRa8 pixel format Heatmap representation of the DoLP
       * (Degree of Linear Polarization). Resulting polarization values are
       * normalized between 0 and 1. */
      DOLP_HEATMAP,
      /** Extracts a Mono8 pixel format image representation of the AoP (Angle
         of Polarization). */
      AOP_GRAYSCALE,
      /** Extracts a BGRa8 pixel format Heatmap representation of the AoP (Angle
       * of Polarization).
...
enum DEPRECATED_CLASS(
    "This enum has been deprecated. Image scaling can now be applied through "
    "specific functions defined in the ImageUtility class.")
    PolarizationResolution {
      /** Quarter Resolution. */
      QUARTER_RESOLUTION,
      /** Full Resolution. */
      FULL_RESOLUTION
...
enum DEPRECATED_CLASS("This enum has been deprecated. Please use HeatmapColor "
                      "in the ImageUtilityHeatmap class.") HeatMapColor {
  HEATMAP_BLACK = 1,
  HEATMAP_BLUE = 2,
  HEATMAP_CYAN = 3,
  HEATMAP_GREEN = 4,
...
enum ImageStatus {
  ...
  IMAGE_NO_ERROR =
      0, /**< Image is returned from GetNextImage() call without any errors. */
  IMAGE_CRC_CHECK_FAILED = 1, /**< Image failed CRC check. */
  IMAGE_DATA_OVERFLOW =
      2, /**< Received more data than the size of the image. */
  ...
  IMAGE_NO_SYSTEM_RESOURCES = 12   /**< Image cannot be processed due to lack of
                                   system   resources. */
};
...
enum SpinnakerLogLevel {
  ...
  LOG_LEVEL_ERROR =
      300, // Failures that are non-recoverable without user intervention.
  LOG_LEVEL_WARN =
      400, // Failures that are recoverable without user intervention.
  LOG_LEVEL_NOTICE =
      500, // Events such as camera arrival and removal, initialization and
           // deinitialization, starting and stopping image acquisition, and
           // feature modification.
  LOG_LEVEL_INFO = 600, // Information about recurring events that are generated
                        // regularly such as information on individual images.
  LOG_LEVEL_DEBUG =
      700, // Information that can be used to troubleshoot the system.
...
enum ActionCommandStatus {
  ...
  ACTION_COMMAND_STATUS_NO_REF_TIME =
      0x8013, /* The device is not synchronized to a master clock to be used as
                 time reference. Typically used when scheduled action commands
                 cannot be scheduled for a future time since the reference time
                 coming from IEEE 1588 is not locked. */
  ACTION_COMMAND_STATUS_OVERFLOW =
      0x8015, /* Returned when the scheduled action commands queue is full and
                 the device cannot accept the additional request. */
  ACTION_COMMAND_STATUS_ACTION_LATE =
      0x8016, /* The requested scheduled action command was requested at a point
                 in time that is in the past. */
  ACTION_COMMAND_STATUS_ERROR =
      0x8FFF /* Generic Error. Try enabling the Extended Status Code 2.0 bit on
                gvcp configuration register in order to receive more
                meaningful/detailed acknowledge messages from the device. */
};

/** Action Command Result */
struct ActionCommandResult {
  unsigned int DeviceAddress; /* IP Address of device that responded to Action
                                 Command. */
