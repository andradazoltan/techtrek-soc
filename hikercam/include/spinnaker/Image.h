...
public:
   ...
   */
  static ImagePtr Create(size_t width, size_t height, size_t offsetX,
                         size_t offsetY,
                         Spinnaker::PixelFormatEnums pixelFormat, void *pData);
   ...
   */
  static void
  SetDefaultColorProcessing(ColorProcessingAlgorithm colorAlgorithm);
   ...
   * A heatmap image reinterprets monochrome data by mapping the luminosity of
   * each pixel to a color value defined in the heatmap color gradient. The
   * created image can be modified by changing the color gradient and heatmap
   * range from the accompanying functions. The source image is required to be
   * Mono8 or Mono16 pixel format.
   *
   ...
   */
  DEPRECATED_FUNC("This function has been deprecated. Please use the "
                  "equivalent function in the ImageUtilityHeatmap class.",
   ...
   * created image can be modified by changing the color gradient and heatmap
   * range from the accompanying functions. The source image is required to be
   * Mono8 or Mono16 pixel format. The destination is required to be
   * initialized, RGB8 or RGB16 pixel format, and have the same width, height, x
   * offset, and y offset as the source image.
   *
   * @param srcImage The source image from which to create the heatmap
   * @param destImage The destination image in which to store the created
   * heatmap
   *
   * @see SetHeatMapRange()
   * @see SetHeatMapColorGradient()
  ...
  /**
   * Sets the heatmap gradient color vector to the new desired range between
   * HEATMAP_BLACK and HEATMAP_WHITE.
   *
   * @param newLowColor New color at which to begin the gradient.
   * @param newHighColor New color at which to end the gradient.
   */
  DEPRECATED_FUNC(
      "This function has been deprecated. Please use the equivalent function "
      "in the ImageUtilityHeatmap class.",
  ...
  /**
   * Sets the high and low values used to determine which grayscale values are
   * converted to a color 'heatmap' representation. Acceptable values range from
   * 0 to 100.
   *
   ...
   * @param colorAlgorithm Optional color processing algorithm for producing the
   * converted image
   ...
   *
   * @see Create(size_t width, size_t height, size_t offsetX, size_t offsetY,
   * Spinnaker::PixelFormatEnums pixelFormat, void* pData)
   *
   * @param destinationImage Destination image where the converted output result
   * will be stored.
   * @param format Output format of the converted image.
   * @param colorAlgorithm Optional color processing algorithm for producing the
   * converted image.
   */
  void Convert(ImagePtr destinationImage, Spinnaker::PixelFormatEnums format,
               ColorProcessingAlgorithm colorAlgorithm = DEFAULT) const;
   ...
   */
  void ResetImage(size_t width, size_t height, size_t offsetX, size_t offsetY,
   ...
   */
  double GetDataAbsoluteMax() const;
   ...
   */
  double GetDataAbsoluteMin() const;
   ...
   * the end of each line to facilitate alignment in buffers. This information
   ...
   * the end of each image to facilitate alignment in buffers. This information
   * is retrieved from the Transport Layer Image format headers.  It is
   * retrieved on a per image basis.
   *
   ...
   * information is retrieved from the Transport Layer Image format headers.  It
   * is retrieved on a per image basis.
   *
   ...
   * transmitted. This information is retrieved from the Transport Layer Image
   * format headers.  It is retrieved on a per image basis.
   *
   ...
   * interpreted. To understand how to interpret this value it is necessary to
   * know what the transport layer namespace is. This can be retrieved through
   ...
   * string is a valid SFNC name that maps to the underlying TL specific pixel
   * format. This is the most generic way to identify the pixel format of the
   * image.
   *
  ...
  /**
   * Returns the size of valid data in the image payload.  This is the actual
   * amount of data read from the device. A user created image has a payload
   * size of zero. GetBufferSize() returns the total size of bytes allocated for
   * the image.
   *
   * @see GetBufferSize()
   *
   ...
   *
   * @return Returns true if image contains ImageCRC checksum from chunk data
   * and false otherwise.
   */
  bool HasCRC() const;
   ...
   *
   * @return Returns true if computed checksum matches with the chunk data's CRC
   * and false otherwise.
   */
  bool CheckCRC() const;
  ...
  DEPRECATED_FUNC("This function has been deprecated. Please use more specific "
                  ...
                  ImagePtr ExtractPolarization(
                      const PolarizationAlgorithm polarizationAlgorithm,
                      const PolarizationResolution resolution) const);

  /**
   * Returns the polarization values associated with an extracted polarization
   * image. Note that standard quadrants (QUADRANT_I0_GRAYSCALE -
   * QUADRANT_I135_GRAYSCALE) do not provide polarization values.
   *
...
protected:
  ...
  Image(size_t width, size_t height, size_t offsetX, size_t offsetY,
        Spinnaker::PixelFormatEnums pixelFormat, void *pData);

  ImagePtr CreateShared() const;
  void DeepCopy(const Image &pSrcImage);
  void Convert(Spinnaker::PixelFormatEnums format, Image &pDestImage,
               ColorProcessingAlgorithm colorAlgorithm = DEFAULT) const;
};
