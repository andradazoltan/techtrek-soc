...
Exposure time in microseconds when Exposure Mode is Timed.
   ...
   * Acquisition Frame Rate it is because the Exposure Time is greater than the
   * frame time. Visibility:
   */
  GenApi::IFloat &AcquisitionResultingFrameRate;

  /**
   * Description: Controls the rate (in Hertz) at which the Lines in a Frame are
   * captured. Visibility:
   */
...
Sets the operation mode of the Exposure.
  ...
  /**
   * Description: Sets the acquisition mode of the device. Continuous: acquires
   * images continuously. Multi Frame: acquires a specified number of images
   * before stopping acquisition. Single Frame: acquires 1 image before stopping
...
Controls whether or not trigger is active.
   ...
   * control the frame rate. Visibility:
   */
...
AcquisitionMode is MultiFrame and ignored if AcquisitionMode is Single.
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Exposure End Event. Visibility:
   */
  GenApi::IInteger &EventExposureEndFrameID;

  /**
   * Description: Returns the unique identifier of the Exposure End type of
   * Event. Visibility:
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Error Event. Visibility:
   */
  ...
  /**
   * Description: Returns the unique identifier of the Serial Port Receive type
   * of Event. Visibility:
   */
  ...
  /**
   * Description: Returns the length of the received serial data that was
   * included in the event payload. Visibility:
   */
  ...
  /**
   * Description: Controls the row of the truth table to access in the selected
   * LUT. Visibility:
   */
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<LogicBlockLUTInputActivationEnums>
      &LogicBlockLUTInputActivation;

  /**
   * Description: Controls which LogicBlockLUT Input Source & Activation to
   * access. Visibility:
   */
  GenApi::IEnumerationT<LogicBlockLUTInputSelectorEnums>
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<LogicBlockLUTInputSourceEnums>
      &LogicBlockLUTInputSource;

  /**
   * Description: Controls the output column of the truth table for the selected
   * LogicBlockLUTRowIndex. Visibility:
   */
...
Enabling/disabling RGB to YUV can only be done by changing pixel format.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<ColorTransformationSelectorEnums>
...
selected Color Transformation module
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<ColorTransformationValueSelectorEnums>
...
Device's serial number. This string is a unique identifier of the device.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<DeviceRegistersEndiannessEnums>
...
Indicates the number of event channels supported by the device.
  ...
  /**
   * Description: Latches the current timestamp counter into
   * TimestampLatchValue. Visibility:
   */
  ...
  /**
   * Description: This is a command that immediately resets and reboots the
   * device. Visibility:
   */
...
Selects the power supply source to control or read.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<DevicePowerSupplySelectorEnums>
...
Selects the location within the device, where the temperature will be measured.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<DeviceTemperatureSelectorEnums>
...
Indicates the current voltage of the selected power supply (V).
  ...
  /**
   * Description: Controls the LED behaviour: Inactive (off), Active (current
   * status), or Error Status (off unless an error occurs). Visibility:
   */
...
meet the target. This mode is used to capture fast motion.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<AutoExposureControlPriorityEnums>
...
The highest value in percentage that the target mean may reach.
  ...
  /**
   * Description:
This is the user-specified target grey level (image mean) to apply to the
current image. Note that the target grey level is in the linear domain before
...
is Normal.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<AutoExposureMeteringModeEnums>
...
If the value is too small, it may cause the system to be unstable.
  ...
  /**
   * Description:
The EV compensation value used in the exposure compensation. This allows you to
adjust the resultant image intensity with one control. A positive value makes
the image brighter. A negative value makes the image darker. Range from -3 to 3
with a step of 1/3. Default = 0.
...
linear domain before gamma correction is applied.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<AutoExposureTargetGreyValueAutoEnums>
...
lighting is selected, metering modes are available.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<AutoExposureLightingModeEnums>
...
BalanceRatio.
  ...
  /**
   * Description: Controls whether the DHCP IP configuration scheme is activated
   * on the given logical link. Visibility:
   */
  ...
  /**
   * Description: Controls the delay (in GEV timestamp counter unit) to insert
   * between each packet for this stream channel. This can be used as a crude
   * flow-control mechanism if the application or the network infrastructure
   * cannot keep up with the packets coming from the device. Visibility:
   */
  GenApi::IInteger &GevSCPD;

  /**
   * Description: Indicates the number of timestamp ticks in 1 second (frequency
   * in Hz). Visibility:
   */
  GenApi::IInteger &GevTimestampTickFrequency;

  /**
   * Description: Specifies the stream packet size (in bytes) to send on this
   * channel. Visibility:
   */
  GenApi::IInteger &GevSCPSPacketSize;

  /**
   * Description: Reports the default gateway IP address to be used on the given
   * logical link. Visibility:
   */
  GenApi::IInteger &GevCurrentDefaultGateway;

  /**
   * Description: Enables the camera to continue to stream, for this stream
   * channel, if its control channel is closed or regardless of the reception of
   * any ICMP messages (such as destination unreachable messages). Visibility:
   */
  ...
  /**
   * Description: The state of this feature is copied into the "do not fragment"
   * bit of the IP header of each stream packet. Visibility:
   */
  ...
  /**
   * Description: Indicates the longest GVCP command execution time before the
   * device returns a PENDING_ACK in milliseconds. Visibility:
   */
  ...
  /**
   * Description: Controls the Persistent subnet mask associated with the
   * Persistent IP address on this logical link. Visibility:
   */
  ...
  /**
   * Description: Controls the port of the selected channel to which a GVSP
   * transmitter must send data stream or the port from which a GVSP receiver
   * may receive data stream. Visibility:
   */
  ...
  /**
   * Description: Controls whether the Link Local Address IP configuration
   * scheme is activated on the given logical link. Visibility:
   */
  ...
  /**
   * Description: Enables the IEEE 1588 Precision Time Protocol to control the
   * timestamp register. Visibility:
   */
  GenApi::IBoolean &GevIEEE1588;

  /**
   * Description: Enables cameras to use the extended chunk data payload type
   * for this stream channel. Visibility:
   */
  ...
  /**
   * Description: Controls whether the PersistentIP configuration scheme is
   * activated on the given logical link. Visibility:
   */
  GenApi::IBoolean &GevCurrentIPConfigurationPersistentIP;

  /**
   * Description: Indicates the expected accuracy of the device clock when it is
   * the grandmaster, or in the event it becomes the grandmaster. Visibility:
   */
  GenApi::IEnumerationT<GevIEEE1588ClockAccuracyEnums>
  ...
  /**
   * Description: Controls the destination IP address of the selected stream
   * channel to which a GVSP transmitter must send data stream or the
   * destination IP address from which a GVSP receiver may receive data stream.
   * Visibility:
   */
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<GevSupportedOptionSelectorEnums>
  ...
  /**
   * Description: Indicates the number of physical network interfaces supported
   * by this device. Visibility:
   */
  ...
  /**
   * Description: Provides the number of bytes transferred for each image or
   * chunk on the stream channel. Visibility:
   */
...
and Digital are read-only.
  ...
  /**
   * Description:
Enables/disables the auto sharpening feature. When enabled, the camera
automatically determines the sharpening threshold based on the noise level of
the camera.
  ...
  /**
   * Description:
Enable the black level auto clamping feature which performing dark current
compensation.
...
When "Sharpening Auto" is enabled, this is determined automatically by the
device. The threshold is specified as a fraction of the total intensity range,
and ranges from 0 to 0.25. A threshold higher than 25% produces little to no
difference than 25%. High thresholds sharpen only areas with significant
...
The sharpened amount is proportional to the difference between a pixel and its
neighbors. A negative value smooths out the difference, while a positive value
amplifies the difference. You can boost by a maximum of 8x, but smoothing is
limited to 1x  (in float). Default value: 2.0
...
Controls the offset of the video signal in camera specific units.
  ...
  /**
   * Description:
Controls the offset of the element to access in the defective pixel location
table.
  ...
  /**
   * Description:
  Restores the Defective Pixel Table to its factory default state, which was
  calibrated during manufacturing. This permanently overwrites any changes made
  to the defect table.
  ...
  /**
   * Description:
Returns the Y coordinate of the defective pixel at DefectTableIndex within the
defective pixel table. Changes made do not take effect in captured images until
the command DefectTableApply is written.
  ...
  /**
   * Description:
Returns the X coordinate of the defective pixel at DefectTableIndex within the
defective pixel table. Changes made do not take effect in captured images until
the command DefectTableApply is written.
...
The number of defective pixel locations in the current table.
  ...
  /**
   * Description:
Saves the User Set specified by UserSetSelector to the non-volatile memory of
the device.
...
Selects the feature User Set to load, save or configure.
  ...
  /**
   * Description:
Loads the User Set specified by UserSetSelector to the device and makes it
active.
...
is restarted.
  ...
  /**
   * Description: This feature controls the baud rate used by the selected
   * serial port. Visibility:
   */
  GenApi::IEnumerationT<SerialPortBaudRateEnums> &SerialPortBaudRate;

  /**
   * Description: This feature controls the number of data bits used by the
   * selected serial port.  Possible values that can be used are between 5
   * and 9. Visibility:
   */
  GenApi::IInteger &SerialPortDataBits;

  /**
   * Description: This feature controls the parity used by the selected serial
   * port. Visibility:
   */
  GenApi::IEnumerationT<SerialPortParityEnums> &SerialPortParity;

  /**
   * Description: >Returns the maximum number of characters in the serial port
   * transmit queue. Visibility:
   */
  GenApi::IInteger &SerialTransmitQueueMaxCharacterCount;

  /**
   * Description: Returns the number of characters currently in the serial port
   * receive queue. Visibility:
   */
  ...
  /**
   * Description: This feature controls the number of stop bits used by the
   * selected serial port. Visibility:
   */
  GenApi::IEnumerationT<SerialPortStopBitsEnums> &SerialPortStopBits;

  /**
   * Description: This is a command that clears the device serial port receive
   * queue. Visibility:
   */
  GenApi::ICommand &SerialReceiveQueueClear;

  /**
   * Description: Returns the number of framing errors that have occurred on the
   * serial port. Visibility:
   */
  ...
  /**
   * Description: Returns the number of parity errors that have occurred on the
   * serial port. Visibility:
   */
  GenApi::IInteger &SerialReceiveParityErrorCount;

  /**
   * Description: Specifies the physical input Line on which to receive serial
   * data. Visibility:
   */
...
Display whether the current sequencer configuration is valid to run.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<SequencerConfigurationValidEnums>
...
Specifies the activation mode of the sequencer trigger.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<SequencerTriggerActivationEnums>
...
Controls whether or not a sequencer is in configuration mode.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<SequencerConfigurationModeEnums>
...
Enables the selected feature and makes it active in all sequencer sets.
  ...
  /**
   * Description: Specifies the number of data blocks (images) that the device
   * should stream before stopping. This feature is only active if the Transfer
   * Operation Mode is set to Multi Block. Visibility:
   */
  GenApi::IInteger &TransferBlockCount;

  /**
   * Description: Starts the streaming of data blocks (images) out of the
   * device. This feature is available when the Transfer Control Mode is set to
   * User Controlled. Visibility:
   */
  GenApi::ICommand &TransferStart;

  /**
   * Description: Returns the maximum number of data blocks (images) in the
   * transfer queue Visibility:
   */
  GenApi::IInteger &TransferQueueMaxBlockCount;

  /**
   * Description: Returns number of data blocks (images) currently in the
   * transfer queue. Visibility:
   */
  ...
  /**
   * Description: Selects the operation mode of the transfer. Continuous is
   * similar to Basic/Automatic but you can start/stop the transfer while
   * acquisition runs independently. Multi Block transmits a specified number of
   * blocks and then stops. Visibility:
   */
  GenApi::IEnumerationT<TransferOperationModeEnums> &TransferOperationMode;

  /**
   * Description: Stops the streaming of data block (images). The current block
   * transmission is completed. This feature is available when the Transfer
   * Control Mode is set to User Controlled. Visibility:
   */
  GenApi::ICommand &TransferStop;

  /**
   * Description: Returns number of images that have been lost before being
   transmitted because the transmit queue hasn't been cleared fast enough.
  ...
  /**
   * Description: Selects the control method for the transfers. Basic and
   * Automatic start transmitting data as soon as there is enough data to fill a
   * link layer packet. User Controlled allows you to directly control the
   * transfer of blocks. Visibility:
   */
  ...
  /**
   * Description: Activates the inclusion of Chunk data in the payload of the
   * image. Visibility:
   */
  GenApi::IBoolean &ChunkModeActive;

  /**
   * Description: Returns the status of all the I/O lines at the end of exposure
   * event. Visibility:
   */
  ...
  /**
   * Description: Returns the index of the active set of the running sequencer
   * included in the payload. Visibility:
   */
  ...
  /**
   * Description: Enables the inclusion of the selected Chunk data in the
   * payload of the image. Visibility:
   */
  ...
  /**
   * Description: Controls the Offset of the mapping between the device file
   * storage and the FileAccessBuffer. Visibility:
   */
  GenApi::IInteger &FileAccessOffset;

  /**
   * Description: Controls the Length of the mapping between the device file
   * storage and the FileAccessBuffer. Visibility:
   */
  ...
  /**
   * Description:
This is a command that executes the selected file operation on the selected
file.
...
writting or both. This must be set before opening the file.
  ...
  /**
   * Description: Represents the file operation result. For Read or Write
   * operations, the number of successfully read/written bytes is returned.
   * Visibility:
   */
...
BinningVertical features.
  ...
  /**
   * Description:
  Minimum value that can be returned during the digitization process. This
  corresponds to the darkest value of the camera. For color cameras, this
  returns the smallest value that each color component can take.
  ...
  /**
   * Description:
  Maximum value that can be returned during the digitization process. This
  corresponds to the brightest value of the camera. For color cameras, this
  returns the biggest value that each color component can take.
...
Selects which test pattern generator is controlled by the TestPattern feature.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<TestPatternGeneratorSelectorEnums>
      &TestPatternGeneratorSelector;

  /**
   * Description: Reports the ratio between the uncompressed image size and
   * compressed image size. Visibility:
   */
  GenApi::IFloat &CompressionRatio;

  /**
   * Description:
  Horizontally flips the image sent by the device. The region of interest is
  applied after flipping. For color cameras the bayer pixel format is affected.
  For example, BayerRG16 changes to BayerGR16.
  ...
  /**
   * Description:
  Vertically flips the image sent by the device. The region of interest is
  applied after flipping. For color cameras the bayer pixel format is affected.
  For example, BayerRG16 changes to BayerGB16.
...
source.
  ...
  /**
   * Description:
Maximum width of the image (in pixels). The dimension is calculated after
horizontal binning. WidthMax does not take into account the current Region of
interest (Width or OffsetX).
...
Discard.  Average should be achieved via Binning.
   ...
   * Visibility:
   */
  GenApi::IEnumerationT<DecimationHorizontalModeEnums>
...
Horizontal offset from the origin to the ROI (in pixels).
  ...
  /**
   * Description: Maximum height of the image (in pixels). This dimension is
calculated after vertical binning. HeightMax does not take into account the
current Region of interest (Height or OffsetY).
   * Visibility:
   */
  ...
  /**
   * Description:
Horizontal decimation of the image.  This reduces the horizontal resolution
(width) of the image by only retaining a single pixel within a window whose size
is the decimation factor specified here. A value of 1 indicates that no
horizontal decimation is performed by the camera. This value must be 1 for
binning to be active.
  ...
  /**
   * Description: Selects which decimation layer is controlled by the
   * DecimationHorizontal and DecimationVertical features. Visibility:
   */
...
format mode (i.e. mono).
  ...
  /**
   * Description: Controls whether lossless compression adapts to the image
   * content.  If disabled, a fixed encoding table is used. Visibility:
   */
  ...
  /**
   * Description:
Vertical decimation of the image.  This reduces the vertical resolution (height)
of the image by only retaining a single pixel within a window whose size is the
decimation factor specified here. A value of 1 indicates that no vertical
decimation is performed by the camera. This value must be 1 for binning to be
active.
...
Average should be achieved via Binning.
  ...
  /**
   * Description: Internally generated 3.3V rail. Enable to supply external
   * circuits with power. This is different than standard logic outputs in that
   * it is comparatively slow to switch but can supply a more significant amount
   * of power. This is only available on some pins. Visibility:
   */
  GenApi::IBoolean &V3_3Enable;

  /**
   * Description: Controls if the physical Line is used to Input or Output a
   * signal. Visibility:
   */
  GenApi::IEnumerationT<LineModeEnums> &LineMode;

  /**
   * Description: Selects which internal acquisition or I/O source signal to
   * output on the selected line. LineMode must be Output. Visibility:
   */
  GenApi::IEnumerationT<LineSourceEnums> &LineSource;

  /**
   * Description: Selects the kind of input filter to configure: Deglitch or
   * Debounce. Visibility:
   */
  GenApi::IEnumerationT<LineInputFilterSelectorEnums> &LineInputFilterSelector;

  /**
   * Description: Value of the selected user output, either logic high (enabled)
   * or logic low (disabled). Visibility:
   */
  GenApi::IBoolean &UserOutputValue;

  /**
   * Description: Returns the current status of all the user output status bits
   * in a hexadecimal representation (UserOutput 0 status corresponds to bit 0,
   * UserOutput 1 status with bit 1, etc). This allows simultaneous reading of
   * all user output statuses at once. Visibility:
   */
  GenApi::IInteger &UserOutputValueAll;

  /**
   * Description: Selects which bit of the User Output register is set by
   * UserOutputValue. Visibility:
   */
  GenApi::IEnumerationT<UserOutputSelectorEnums> &UserOutputSelector;

  /**
   * Description: Returns the current status of the selected input or output
   * Line Visibility:
   */
  GenApi::IBoolean &LineStatus;

  /**
   * Description: Displays the current electrical format of the selected
   * physical input or output Line. Visibility:
   */
  GenApi::IEnumerationT<LineFormatEnums> &LineFormat;

  /**
   * Description: Returns the current status of all the line status bits in a
   * hexadecimal representation (Line 0 status corresponds to bit 0, Line 1
   * status with bit 1, etc). This allows simultaneous reading of all line
   * statuses at once. Visibility:
   */
  GenApi::IInteger &LineStatusAll;

  /**
   * Description: Selects the physical line (or pin) of the external device
   * connector to configure Visibility:
   */
  ...
  /**
   * Description: Controls the inversion of the signal of the selected input or
   * output line. Visibility:
   */
  GenApi::IBoolean &LineInverter;

  /**
   * Description: Filter width in microseconds for the selected line and filter
   * combination Visibility:
   */
  GenApi::IFloat &LineFilterWidth;

  /**
   * Description: Selects the activation mode of the trigger to start the
   * Counter. Visibility:
   */
  GenApi::IEnumerationT<CounterTriggerActivationEnums>
  ...
  /**
   * Description: Sets the delay (or number of events) before the CounterStart
   * event is generated. Visibility:
   */
  GenApi::IInteger &CounterDelay;

  /**
   * Description: Selects the signal that will be the source to reset the
   * Counter. Visibility:
   */
  ...
  /**
   * Description: Selects the activation mode of the event to increment the
   * Counter. Visibility:
   */
  GenApi::IEnumerationT<CounterEventActivationEnums> &CounterEventActivation;

  /**
   * Description: Sets the duration (or number of events) before the CounterEnd
   * event is generated. Visibility:
   */
  GenApi::IInteger &CounterDuration;

  /**
   * Description: Selects the Activation mode of the Counter Reset Source
   * signal. Visibility:
   */
  ...
  /**
   * Description: Major version of the Standard Features Naming Convention that
   * was used to create the device's GenICam XML. Visibility: Beginner
   */
  GenApi::IInteger &DeviceSFNCVersionMajor;

  /**
   * Description: Minor version of the Standard Features Naming Convention that
   * was used to create the device's GenICam XML. Visibility: Beginner
   */
  GenApi::IInteger &DeviceSFNCVersionMinor;

  /**
   * Description: Sub minor version of Standard Features Naming Convention that
   * was used to create the device's GenICam XML. Visibility: Beginner
   */
  ...
  /**
   * Description: Indicates the major version number of the GenICam XML file of
   * the selected manifest entry. Visibility: Guru
   */
  GenApi::IInteger &DeviceManifestXMLMajorVersion;

  /**
   * Description: Indicates the minor version number of the GenICam XML file of
   * the selected manifest entry. Visibility: Guru
   */
  GenApi::IInteger &DeviceManifestXMLMinorVersion;

  /**
   * Description: Indicates the subminor version number of the GenICam XML file
   * of the selected manifest entry. Visibility: Guru
   */
  GenApi::IInteger &DeviceManifestXMLSubMinorVersion;

  /**
   * Description: Indicates the major version number of the schema file of the
   * selected manifest entry. Visibility: Guru
   */
  GenApi::IInteger &DeviceManifestSchemaMajorVersion;

  /**
   * Description: Indicates the minor version number of the schema file of the
   * selected manifest entry. Visibility: Guru
   */
  GenApi::IInteger &DeviceManifestSchemaMinorVersion;

  /**
   * Description: Indicates the first URL to the GenICam XML device description
   * file of the selected manifest entry. Visibility: Guru
   */
  GenApi::IString &DeviceManifestPrimaryURL;

  /**
   * Description: Indicates the second URL to the GenICam XML device description
   * file of the selected manifest entry. Visibility: Guru
   */
  ...
  /**
   * Description: Indicates the speed of transmission of the specified
   * Connection Visibility: Expert
   */
  ...
  /**
   * Description: Controls if the DeviceLinkThroughputLimit is active. When
   * disabled, lower level TL specific features are expected to control the
   * throughput. When enabled, DeviceLinkThroughputLimit controls the overall
   * throughput. Visibility: Expert
   */
  GenApi::IEnumerationT<DeviceLinkThroughputLimitModeEnums>
      &DeviceLinkThroughputLimitMode;

  /**
   * Description: Returns the number of physical connection of the device used
   * by a particular Link. Visibility: Beginner
   */
  ...
  /**
   * Description: Indicates the command timeout of the specified Link. This
   * corresponds to the maximum response time of the device for a command sent
   * on that link. Visibility: Guru
   */
  ...
  /**
   * Description: Index of device's Link to use for streaming the specifed
   * stream channel. Visibility: Guru
   */
   ...
   * Visibility: Guru
   */
  GenApi::IEnumerationT<DeviceStreamChannelEndiannessEnums>
      &DeviceStreamChannelEndianness;

  /**
   * Description: Specifies the stream packet size, in bytes, to send on the
   * selected channel for a Transmitter or specifies the maximum packet size
   * supported by a receiver. Visibility: Expert
   */
  GenApi::IInteger &DeviceStreamChannelPacketSize;

  /**
   * Description: Indicate to the device and GenICam XML to get ready for
   * persisting of all streamable features. Visibility: Guru
   */
  ...
  /**
   * Description: Prepare the device for registers streaming without checking
   * for consistency. Visibility: Guru
   */
  GenApi::ICommand &DeviceRegistersStreamingStart;

  /**
   * Description: Announce the end of registers streaming. This will do a
   * register set validation for consistency and activate it. This will also
   * update the DeviceRegistersValid flag. Visibility: Guru
   */
  GenApi::ICommand &DeviceRegistersStreamingEnd;

  /**
   * Description: Perform the validation of the current register set for
   * consistency. This will update the DeviceRegistersValid flag. Visibility:
   * Expert
   */
   ...
   * Visibility: Expert
   */
  GenApi::IEnumerationT<DeviceSerialPortSelectorEnums>
      &DeviceSerialPortSelector;

  /**
   * Description: This feature controls the baud rate used by the selected
   * serial port. Visibility: Expert
   */
  GenApi::IEnumerationT<DeviceSerialPortBaudRateEnums>
   ...
   * camera A/D conversion stage. Visibility: Expert
   */
  GenApi::IEnumerationT<SensorDigitizationTapsEnums> &SensorDigitizationTaps;

  /**
   * Description: Selects the Region of interest to control. The RegionSelector
   * feature allows devices that are able to extract multiple regions out of an
   * image, to configure the features of those individual regions independently.
   * Visibility: Beginner
   */
  GenApi::IEnumerationT<RegionSelectorEnums> &RegionSelector;

  /**
   * Description: Controls if the selected Region of interest is active and
   * streaming. Visibility: Beginner
   */
  ...
  /**
   * Description: Total number of bytes between 2 successive lines. This feature
   * is used to facilitate alignment of image data. Visibility: Expert
   */
  GenApi::IInteger &LinePitch;

  /**
   * Description: Select the pixel format for which the information will be
   * returned. Visibility: Guru
   */
  GenApi::IEnumerationT<PixelFormatInfoSelectorEnums> &PixelFormatInfoSelector;

  /**
   * Description: Returns the value used by the streaming channels to identify
   * the selected pixel format. Visibility: Guru
   */
  ...
  /**
   * Description: Two rate controlling options are offered: fixed bit rate or
   * fixed quality. The exact implementation to achieve one or the other is
   * vendor-specific. Visibility: Expert
   */
  GenApi::IEnumerationT<ImageCompressionRateOptionEnums>
  ...
  /**
   * Description: When JPEG is selected as the compression format, a device
   * might optionally offer better control over JPEG-specific options through
   * this feature. Visibility: Expert
   */
  GenApi::IEnumerationT<ImageCompressionJPEGFormatOptionEnums>
      &ImageCompressionJPEGFormatOption;

  /**
   * Description: Aborts the Acquisition immediately. This will end the capture
   * without completing the current Frame or waiting on a trigger. If no
   * Acquisition is in progress, the command is ignored. Visibility: Expert
   */
  GenApi::ICommand &AcquisitionAbort;

  /**
   * Description: Arms the device before an AcquisitionStart command. This
   * optional command validates all the current features for consistency and
   * prepares the device for a fast start of the Acquisition. Visibility: Expert
   */
  GenApi::ICommand &AcquisitionArm;

  /**
   * Description: Selects the internal acquisition signal to read using
   * AcquisitionStatus. Visibility: Expert
   */
  GenApi::IEnumerationT<AcquisitionStatusSelectorEnums>
      &AcquisitionStatusSelector;

  /**
   * Description: Reads the state of the internal acquisition signal selected
   * using AcquisitionStatusSelector. Visibility: Expert
   */
  ...
  /**
   * Description: Specifies a multiplication factor for the incoming trigger
   * pulses. It is used generally used in conjunction with TriggerDivider to
   * control the ratio of triggers that are accepted. Visibility: Expert
   */
  ...
  /**
   * Description: Selects which exposure time is controlled by the ExposureTime
   * feature. This allows for independent control over the exposure components.
   * Visibility: Beginner
   */
  GenApi::IEnumerationT<ExposureTimeSelectorEnums> &ExposureTimeSelector;

  /**
   * Description: Sets the mode for automatic gain balancing between the sensor
   * color channels or taps. The gain coefficients of each channel or tap are
   * adjusted so they are matched. Visibility: Beginner
   */
  GenApi::IEnumerationT<GainAutoBalanceEnums> &GainAutoBalance;

  /**
   * Description: Controls the mode for automatic black level adjustment. The
   * exact algorithm used to implement this adjustment is device-specific.
   * Visibility: Expert
   */
  GenApi::IEnumerationT<BlackLevelAutoEnums> &BlackLevelAuto;

  /**
   * Description: Controls the mode for automatic black level balancing between
   * the sensor color channels or taps. The black level coefficients of each
   * channel are adjusted so they are matched. Visibility: Expert
   */
  ...
  /**
   * Description: Controls the maximal intensity taken by the video signal
   * before being clipped as an absolute physical value. The video signal will
   * never exceed the white clipping point: it will saturate at that level.
   * Visibility: Expert
   */
  GenApi::IFloat &WhiteClip;

  /**
   * Description: Accesses all the LUT coefficients in a single access without
   * using individual LUTIndex. Visibility: Guru
   */
  GenApi::IRegister &LUTValueAll;

  /**
   * Description: Sets the write mask to apply to the value specified by
   * UserOutputValueAll before writing it in the User Output register. If the
   * UserOutputValueAllMask feature is present, setting the user Output register
   * using UserOutputValueAll will only change the bits that have a
   * corresponding bit in the mask set to one. Visibility: Expert
   */
  GenApi::IInteger &UserOutputValueAllMask;

  /**
   * Description: Does a software reset of the selected Counter and starts it.
   * The counter starts counting events immediately after the reset unless a
   * Counter trigger is active. CounterReset can be used to reset the Counter
   * independently from the CounterResetSource. To disable the counter
   * temporarily, set CounterEventSource to Off. Visibility: Expert
   */
  ...
  /**
   * Description: Sets the duration (in microseconds) of the delay to apply at
   * the reception of a trigger before starting the Timer. Visibility: Expert
   */
  GenApi::IFloat &TimerDelay;

  /**
   * Description: Does a software reset of the selected timer and starts it. The
   * timer starts immediately after the reset unless a timer trigger is active.
   * Visibility: Expert
   */
  GenApi::ICommand &TimerReset;

  /**
   * Description: Reads or writes the current value (in microseconds) of the
   * selected Timer. Visibility: Expert
   */
  ...
  /**
   * Description: Selects the signal which will be the source of the A input of
   * the Encoder. Visibility: Expert
   */
  GenApi::IEnumerationT<EncoderSourceAEnums> &EncoderSourceA;

  /**
   * Description: Selects the signal which will be the source of the B input of
   * the Encoder. Visibility: Expert
   */
  GenApi::IEnumerationT<EncoderSourceBEnums> &EncoderSourceB;

  /**
   * Description: Selects if the count of encoder uses FourPhase mode with
   * jitter filtering or the HighResolution mode without jitter filtering.
   * Visibility: Expert
   */
  GenApi::IEnumerationT<EncoderModeEnums> &EncoderMode;

  /**
   * Description: Sets how many Encoder increment/decrements that are needed
   * generate an Encoder output pulse signal. Visibility: Expert
   */
  GenApi::IInteger &EncoderDivider;

  /**
   * Description: Selects the conditions for the Encoder interface to generate a
   * valid Encoder output signal. Visibility: Expert
   */
  ...
  /**
   * Description: Sets the maximum time interval between encoder counter
   * increments before the status turns to static. Visibility: Expert
   */
  GenApi::IFloat &EncoderTimeout;

  /**
   * Description: Selects the signals that will be the source to reset the
   * Encoder. Visibility: Expert
   */
  GenApi::IEnumerationT<EncoderResetSourceEnums> &EncoderResetSource;

  /**
   * Description: Selects the Activation mode of the Encoder Reset Source
   * signal. Visibility: Expert
   */
   ...
   * The Encoder starts counting events immediately after the reset.
   * EncoderReset can be used to reset the Encoder independently from the
   * EncoderResetSource. Visibility: Expert
   */
  GenApi::ICommand &EncoderReset;

  /**
   * Description: Reads or writes the current value of the position counter of
   * the selected Encoder. Visibility: Expert
   */
  GenApi::IInteger &EncoderValue;

  /**
   * Description: Reads the value of the of the position counter of the selected
   * Encoder when it was reset by a signal or by an explicit EncoderReset
   * command. Visibility: Expert
   */
  ...
  /**
   * Description: Generates a pulse signal that can be used as a software
   * trigger. This command can be used to trigger other modules that accept a
   * SoftwareSignal as trigger source. Visibility: Beginner
   */
  GenApi::ICommand &SoftwareSignalPulse;

  /**
   * Description: Enables the unconditional action command mode where action
  ...
  /**
   * Description: Provides the device key that allows the device to check the
   * validity of action commands. The device internal assertion of an action
   * signal is only authorized if the ActionDeviceKey and the action device key
   * value in the protocol message are equal. Visibility: Guru
   */
  GenApi::IInteger &ActionDeviceKey;

  /**
   * Description: Indicates the size of the scheduled action commands queue.
   * This number represents the maximum number of scheduled action commands that
   * can be pending at a given point in time. Visibility: Guru
   */
   ...
   * action on reception of the action protocol message. Visibility: Guru
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Acquisition Trigger type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Acquisition Trigger Event. Visibility: Expert
   */
  GenApi::IInteger &EventAcquisitionTriggerFrameID;

  /**
   * Description: Returns the unique Identifier of the Acquisition Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Acquisition Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventAcquisitionStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Acquisition End type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Acquisition End Event. Visibility: Expert
   */
  GenApi::IInteger &EventAcquisitionEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Acquisition Transfer
   * Start type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Acquisition Transfer Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventAcquisitionTransferStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Acquisition Transfer End
   * type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Acquisition Transfer End Event. Visibility: Expert
   */
  GenApi::IInteger &EventAcquisitionTransferEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Acquisition Error type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Acquisition Error Event. Visibility: Expert
   */
  GenApi::IInteger &EventAcquisitionErrorFrameID;

  /**
   * Description: Returns the unique Identifier of the FrameTrigger type of
   * Event. It can be used to register a callback function to be notified of the
   * event occurrence. Its value uniquely identifies the type event received.
   * Visibility: Expert
   */
  GenApi::IInteger &EventFrameTrigger;

  /**
   * Description: Returns the Timestamp of the FrameTrigger Event. It can be
   * used to determine precisely when the event occurred. Visibility: Expert
   */
  GenApi::IInteger &EventFrameTriggerTimestamp;

  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the FrameTrigger Event. Visibility: Expert
   */
  GenApi::IInteger &EventFrameTriggerFrameID;

  /**
   * Description: Returns the unique Identifier of the Frame Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Frame Start Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Frame End Event. Visibility: Expert
   */
  GenApi::IInteger &EventFrameEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Frame Burst Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Frame Burst Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventFrameBurstStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Frame Burst End type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Frame Burst End Event. Visibility: Expert
   */
  GenApi::IInteger &EventFrameBurstEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Frame Transfer Start type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Frame Transfer Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventFrameTransferStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Frame Transfer End type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Frame Transfer End Event. Visibility: Expert
   */
  GenApi::IInteger &EventFrameTransferEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Exposure Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Exposure Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventExposureStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Start
   * type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer End
   * type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer End Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Pause
   * type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Pause Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferPauseFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Resume
   * type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Resume Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferResumeFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Block
   * Start type of Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBlockStart;

  /**
   * Description: Returns the Timestamp of the Stream 0 Transfer Block Start
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Block Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBlockStartFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Block
   * End type of Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBlockEnd;

  /**
   * Description: Returns the Timestamp of the Stream 0 Transfer Block End
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Block End Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBlockEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Block
   * Trigger type of Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBlockTrigger;

  /**
   * Description: Returns the Timestamp of the Stream 0 Transfer Block Trigger
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Block Trigger Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBlockTriggerFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer Burst
   * Start type of Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBurstStart;

  /**
   * Description: Returns the Timestamp of the Stream 0 Transfer Burst Start
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Burst Start Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the Timestamp of the Stream 0 Transfer Burst End
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Burst End Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferBurstEndFrameID;

  /**
   * Description: Returns the unique Identifier of the Stream 0 Transfer
   * Overflow type of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Stream 0 Transfer Overflow Event. Visibility: Expert
   */
  GenApi::IInteger &EventStream0TransferOverflowFrameID;

  /**
   * Description: Returns the unique Identifier of the Sequencer Set Change type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Sequencer Set Change Event. Visibility: Expert
   */
  GenApi::IInteger &EventSequencerSetChangeFrameID;

  /**
   * Description: Returns the unique Identifier of the Counter 0 Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Counter 0 Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventCounter0StartFrameID;

  /**
   * Description: Returns the unique Identifier of the Counter 1 Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Counter 1 Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventCounter1StartFrameID;

  /**
   * Description: Returns the unique Identifier of the Counter 0 End type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Counter 0 End Event. Visibility: Expert
   */
  GenApi::IInteger &EventCounter0EndFrameID;

  /**
   * Description: Returns the unique Identifier of the Counter 1 End type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Counter 1 End Event. Visibility: Expert
   */
  GenApi::IInteger &EventCounter1EndFrameID;

  /**
   * Description: Returns the unique Identifier of the Timer 0 Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Timer 0 Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventTimer0StartFrameID;

  /**
   * Description: Returns the unique Identifier of the Timer 1 Start type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Timer 1 Start Event. Visibility: Expert
   */
  GenApi::IInteger &EventTimer1StartFrameID;

  /**
   * Description: Returns the unique Identifier of the Timer 0 End type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Timer 0 End Event. Visibility: Expert
   */
  GenApi::IInteger &EventTimer0EndFrameID;

  /**
   * Description: Returns the unique Identifier of the Timer 1 End type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Timer 1 End Event. Visibility: Expert
   */
  GenApi::IInteger &EventTimer1EndFrameID;

  /**
   * Description: Returns the unique Identifier of the Encoder 0 Stopped type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Encoder 0 Stopped Event. Visibility: Expert
   */
  GenApi::IInteger &EventEncoder0StoppedFrameID;

  /**
   * Description: Returns the unique Identifier of the Encoder 1 Stopped type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Encoder 1 Stopped Event. Visibility: Expert
   */
  GenApi::IInteger &EventEncoder1StoppedFrameID;

  /**
   * Description: Returns the unique Identifier of the Encoder 0 Restarted type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Encoder 0 Restarted Event. Visibility: Expert
   */
  GenApi::IInteger &EventEncoder0RestartedFrameID;

  /**
   * Description: Returns the unique Identifier of the Encoder 1 Restarted type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Encoder 1 Restarted Event. Visibility: Expert
   */
  GenApi::IInteger &EventEncoder1RestartedFrameID;

  /**
   * Description: Returns the unique Identifier of the Line 0 Rising Edge type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Line 0 Rising Edge Event. Visibility: Expert
   */
  GenApi::IInteger &EventLine0RisingEdgeFrameID;

  /**
   * Description: Returns the unique Identifier of the Line 1 Rising Edge type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Line 1 Rising Edge Event. Visibility: Expert
   */
  GenApi::IInteger &EventLine1RisingEdgeFrameID;

  /**
   * Description: Returns the unique Identifier of the Line 0 Falling Edge type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Line 0 Falling Edge Event. Visibility: Expert
   */
  GenApi::IInteger &EventLine0FallingEdgeFrameID;

  /**
   * Description: Returns the unique Identifier of the Line 1 Falling Edge type
   * of Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Line 1 Falling Edge Event. Visibility: Expert
   */
  GenApi::IInteger &EventLine1FallingEdgeFrameID;

  /**
   * Description: Returns the unique Identifier of the Line 0 Any Edge type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Line 0 Any Edge Event. Visibility: Expert
   */
  GenApi::IInteger &EventLine0AnyEdgeFrameID;

  /**
   * Description: Returns the unique Identifier of the Line 1 Any Edge type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Line 1 Any Edge Event. Visibility: Expert
   */
  GenApi::IInteger &EventLine1AnyEdgeFrameID;

  /**
   * Description: Returns the unique Identifier of the Link Trigger 0 type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Link Trigger 0 Event. Visibility: Expert
   */
  GenApi::IInteger &EventLinkTrigger0FrameID;

  /**
   * Description: Returns the unique Identifier of the Link Trigger 1 type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Link Trigger 1 Event. Visibility: Expert
   */
  GenApi::IInteger &EventLinkTrigger1FrameID;

  /**
   * Description: Returns the unique Identifier of the Action Late type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Action Late Event. Visibility: Expert
   */
  GenApi::IInteger &EventActionLateFrameID;

  /**
   * Description: Returns the unique Identifier of the Link Speed Change type of
   * Event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the unique Identifier of the Frame (or image) that
   * generated the Link Speed Change Event. Visibility: Expert
   */
  GenApi::IInteger &EventLinkSpeedChangeFrameID;

  /**
   * Description: Defines the intermediate access buffer that allows the
   * exchange of data between the device file storage and the application.
   * Visibility: Guru
   */
  GenApi::IRegister &FileAccessBuffer;

  /**
   * Description: Controls or returns the number of sources supported by the
   * device. Visibility: Beginner
   */
  ...
  /**
   * Description: Selects which stream transfers are currently controlled by the
   * selected Transfer features. Visibility: Expert
   */
  GenApi::IEnumerationT<TransferSelectorEnums> &TransferSelector;

  /**
   * Description: Number of Block(s) to transfer for each TransferBurstStart
   * trigger. Visibility: Expert
   */
  GenApi::IInteger &TransferBurstCount;

  /**
   * Description: Aborts immediately the streaming of data block(s). Aborting
   * the transfer will result in the lost of the data that is present or
   * currently entering in the block queue. However, the next new block received
   * will be stored in the queue and transferred to the host when the streaming
   * is restarted. If implemented, this feature should be available when the
   * TransferControlMode is set to "UserControlled". Visibility: Expert
   */
  GenApi::ICommand &TransferAbort;

  /**
   * Description: Pauses the streaming of data Block(s). Pausing the streaming
   * will immediately suspend the ongoing data transfer even if a block is
   * partially transfered. The device will resume its transmission at the
   * reception of a TransferResume command. Visibility: Guru
   */
  GenApi::ICommand &TransferPause;

  /**
   * Description: Resumes a data Blocks streaming that was previously paused by
   * a TransferPause command. Visibility: Guru
   */
  ...
  /**
   * Description: Specifies the signal to use as the trigger source for
   * transfers. Visibility: Guru
   */
   ...
   * Visibility: Guru
   */
  GenApi::IEnumerationT<TransferTriggerActivationEnums>
  ...
  /**
   * Description: Reads the status of the Transfer module signal selected by
   * TransferStatusSelector. Visibility: Guru
   */
  GenApi::IBoolean &TransferStatus;

  /**
   * Description: Selects the color component for the control of the
   * TransferStreamChannel feature. Visibility: Guru
   */
  GenApi::IEnumerationT<TransferComponentSelectorEnums>
      &TransferComponentSelector;

  /**
   * Description: Selects the streaming channel that will be used to transfer
   * the selected stream of data. In general, this feature can be omitted and
   * the default streaming channel will be used. Visibility: Guru
   */
  GenApi::IInteger &TransferStreamChannel;

  /**
   * Description: Specifies the unit used when delivering calibrated distance
   * data. Visibility: Beginner
   */
  ...
  /**
   * Description: Controls the Calibration and data organization of the device,
   * naming the coordinates transmitted. Visibility: Expert
   */
   ...
   * Visibility: Expert
   */
  GenApi::IEnumerationT<Scan3dCoordinateSystemReferenceEnums>
      &Scan3dCoordinateSystemReference;

  /**
   * Description: Selects the individual coordinates in the vectors for 3D
   * information/transformation. Visibility: Expert
   */
  GenApi::IEnumerationT<Scan3dCoordinateSelectorEnums>
      &Scan3dCoordinateSelector;

  /**
   * Description: Scale factor when transforming a pixel from relative
   * coordinates to world coordinates. Visibility: Expert
   */
  GenApi::IFloat &Scan3dCoordinateScale;

  /**
   * Description: Offset when transforming a pixel from relative coordinates to
   * world coordinates. Visibility: Expert
   */
  GenApi::IFloat &Scan3dCoordinateOffset;

  /**
   * Description: Enables the definition of a non-valid flag value in the data
   * stream. Note that the confidence output is an alternate recommended way to
   * identify non-valid pixels. Using an Scan3dInvalidDataValue may give
   * processing penalties due to special handling. Visibility: Expert
   */
  GenApi::IBoolean &Scan3dInvalidDataFlag;

  /**
   * Description: Value which identifies a non-valid pixel if
   * Scan3dInvalidDataFlag is enabled. Visibility: Expert
   */
  GenApi::IFloat &Scan3dInvalidDataValue;

  /**
   * Description: Minimum valid transmitted coordinate value of the selected
   * Axis. Visibility: Expert
   */
  GenApi::IFloat &Scan3dAxisMin;

  /**
   * Description: Maximum valid transmitted coordinate value of the selected
   * Axis. Visibility: Expert
   */
   ...
   * Visibility: Expert
   */
  GenApi::IEnumerationT<Scan3dCoordinateTransformSelectorEnums>
      &Scan3dCoordinateTransformSelector;

  /**
   * Description: Specifies the transform value selected. For translations
   * (Scan3dCoordinateTransformSelector = TranslationX/Y/Z) it is expressed in
   * the distance unit of the system, for rotations
   * (Scan3dCoordinateTransformSelector =RotationX/Y/Z) in degrees. Visibility:
   * Expert
   */
  GenApi::IFloat &Scan3dTransformValue;

  /**
   * Description: Sets the index to read a coordinate system reference value
   * defining the transform of a point from the current (Anchor or Transformed)
   * system to the reference system. Visibility: Expert
   */
  GenApi::IEnumerationT<Scan3dCoordinateReferenceSelectorEnums>
      &Scan3dCoordinateReferenceSelector;

  /**
   * Description: Returns the reference value selected. Reads the value of a
   * rotation or translation value for the current (Anchor or Transformed)
   * coordinate system transformation to the Reference system. Visibility:
   * Expert
   */
  ...
  /**
   * Description: Returns the component of the payload image. This can be used
   * to identify the image component of a generic part in a multipart transfer.
   * Visibility: Expert
   */
  ...
  /**
   * Description: Returns the last Timestamp latched with the TimestampLatch
   * command. Visibility: Expert
   */
  GenApi::IInteger &ChunkTimestampLatchValue;

  /**
   * Description: Returns the status of all the I/O lines at the time of the
   * FrameStart internal event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the value of the selected Chunk counter at the time of
   * the FrameStart event. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the value of the selected Timer at the time of the
   * FrameStart internal event. Visibility: Expert
   */
  ...
  /**
   * Description: Index for vector representation of one chunk value per line in
   * an image. Visibility: Expert
   */
  GenApi::IInteger &ChunkScanLineSelector;

  /**
   * Description: Returns the counter's value of the selected Encoder at the
   * time of the FrameStart in area scan mode or the counter's value at the time
  ...
  /**
   * Description: Selects which exposure time is read by the ChunkExposureTime
   * feature. Visibility: Expert
   */
  GenApi::IEnumerationT<ChunkExposureTimeSelectorEnums>
  ...
  /**
   * Description: Returns the unique identifier of the transfer block used to
   * transport the payload. Visibility: Expert
   */
  ...
  /**
   * Description: Returns identifier of the stream channel used to carry the
   * block. Visibility: Expert
   */
  ...
  /**
   * Description: Returns the Coordinate System of the image included in the
   * payload. Visibility: Expert
   */
  GenApi::IEnumerationT<ChunkScan3dCoordinateSystemEnums>
      &ChunkScan3dCoordinateSystem;

  /**
   * Description: Returns the Coordinate System Position of the image included
   * in the payload. Visibility: Expert
   */
  GenApi::IEnumerationT<ChunkScan3dCoordinateSystemReferenceEnums>
   ...
   * Visibility: Expert
   */
  GenApi::IEnumerationT<ChunkScan3dCoordinateSelectorEnums>
  ...
  /**
   * Description: Returns the Offset for the selected coordinate axis of the
   * image included in the payload. Visibility: Expert
   */
  GenApi::IFloat &ChunkScan3dCoordinateOffset;

  /**
   * Description: Returns if a specific non-valid data flag is used in the data
   * stream. Visibility: Expert
   */
  GenApi::IBoolean &ChunkScan3dInvalidDataFlag;

  /**
   * Description: Returns the Invalid Data Value used for the image included in
   * the payload. Visibility: Expert
   */
  GenApi::IFloat &ChunkScan3dInvalidDataValue;

  /**
   * Description: Returns the Minimum Axis value for the selected coordinate
   * axis of the image included in the payload. Visibility: Expert
   */
  GenApi::IFloat &ChunkScan3dAxisMin;

  /**
   * Description: Returns the Maximum Axis value for the selected coordinate
   * axis of the image included in the payload. Visibility: Expert
   */
   ...
   * Visibility: Expert
   */
  GenApi::IEnumerationT<ChunkScan3dCoordinateTransformSelectorEnums>
  ...
  /**
   * Description: Selector to read a coordinate system reference value defining
   * the transform of a point from one system to the other. Visibility: Expert
   */
  GenApi::IEnumerationT<ChunkScan3dCoordinateReferenceSelectorEnums>
      &ChunkScan3dCoordinateReferenceSelector;

  /**
   * Description: Reads the value of a position or pose coordinate for the
   * anchor or transformed coordinate systems relative to the reference point.
   * Visibility: Expert
   */
  GenApi::IFloat &ChunkScan3dCoordinateReferenceValue;

  /**
   * Description: Tests the device's pending acknowledge feature. When this
   * feature is written, the device waits a time period corresponding to the
   * value of TestPendingAck before acknowledging the write. Visibility: Guru
   */
  GenApi::IInteger &TestPendingAck;

  /**
   * Description: This device tap geometry feature describes the geometrical
   * properties characterizing the taps of a camera as presented at the output
   * of the device. Visibility: Expert
   */
  GenApi::IEnumerationT<DeviceTapGeometryEnums> &DeviceTapGeometry;

  /**
   * Description: Controls the principal physical link configuration to use on
   * next restart/power-up of the device. Visibility: Expert
   */
  GenApi::IEnumerationT<GevPhysicalLinkConfigurationEnums>
      &GevPhysicalLinkConfiguration;

  /**
   * Description: Indicates the current physical link configuration of the
   * device. Visibility: Expert
   */
  GenApi::IEnumerationT<GevCurrentPhysicalLinkConfigurationEnums>
  ...
  /**
   * Description: Controls whether incoming PAUSE Frames are handled on the
   * given logical link. Visibility: Expert
   */
  GenApi::IBoolean &GevPAUSEFrameReception;

  /**
   * Description: Controls whether PAUSE Frames can be generated on the given
   * logical link. Visibility: Expert
   */
   ...
   * Visibility: Beginner
   */
  GenApi::IEnumerationT<GevIPConfigurationStatusEnums>
      &GevIPConfigurationStatus;

  /**
   * Description: Indicates the maximum randomized delay the device will wait to
   * acknowledge a discovery command. Visibility: Expert
   */
  GenApi::IInteger &GevDiscoveryAckDelay;

  /**
   * Description: Selects the GigE Vision version to control extended status
   * codes for. Visibility: Guru
   */
  GenApi::IEnumerationT<GevGVCPExtendedStatusCodesSelectorEnums>
  ...
  /**
   * Description: Controls the key to use to authenticate primary application
   * switchover requests. Visibility: Guru
   */
  ...
  /**
   * Description: Enables the alternate IP destination for stream packets resent
   * due to a packet resend request. When True, the source IP address provided
   * in the packet resend command packet is used. When False, the value set in
   * the GevSCDA[GevStreamChannelSelector] feature is used. Visibility: Guru
   */
  GenApi::IBoolean &GevSCCFGPacketResendDestination;

  /**
   * Description: Enables the selected GVSP transmitter to use the single packet
   * per data block All-in Transmission mode. Visibility: Guru
   */
  GenApi::IBoolean &GevSCCFGAllInTransmission;

  /**
   * Description: Reports the number of zones per block transmitted on the
   * selected stream channel. Visibility: Guru
   */
  GenApi::IInteger &GevSCZoneCount;

  /**
   * Description: Reports the transmission direction of each zone transmitted on
   * the selected stream channel. Visibility: Guru
   */
  GenApi::IInteger &GevSCZoneDirectionAll;

  /**
   * Description: Controls whether the selected stream channel multi-zone
   * configuration is locked. When locked, the GVSP transmitter is not allowed
   * to change the number of zones and their direction during block acquisition
   * and transmission. Visibility: Guru
   */
  ...
  /**
   * Description: This Camera Link specific feature describes the configuration
   * used by the camera. It helps especially when a camera is capable of
   * operation in a non-standard configuration, and when the features PixelSize,
   * SensorDigitizationTaps, and DeviceTapGeometry do not provide enough
  ...
  /**
   * Description: This Camera Link specific feature describes the time
   * multiplexing of the camera link connection to transfer more than the
   * configuration allows, in one single clock. Visibility: Expert
   */
  GenApi::IEnumerationT<ClTimeSlotsCountEnums> &ClTimeSlotsCount;

  /**
   * Description: This feature indicates the current and active Link
   * configuration used by the Device. Visibility: Beginner
   */
  GenApi::IEnumerationT<CxpLinkConfigurationStatusEnums>
      &CxpLinkConfigurationStatus;

  /**
   * Description: Provides the Link configuration that allows the Transmitter
   * Device to operate in its default mode. Visibility: Expert
   */
  GenApi::IEnumerationT<CxpLinkConfigurationPreferredEnums>
      &CxpLinkConfigurationPreferred;

  /**
   * Description: This feature allows specifying the Link configuration for the
   * communication between the Receiver and Transmitter Device. In most cases
   * this feature does not need to be written because automatic discovery will
   * set configuration correctly to the value returned by
   * CxpLinkConfigurationPreferred. Note that the currently active configuration
   * of the Link can be read using CxpLinkConfigurationStatus. Visibility:
   * Beginner
   */
  ...
  /**
   * Description: Enables the test mode for an individual physical connection of
   * the Device. Visibility: Expert
   */
  GenApi::IEnumerationT<CxpConnectionTestModeEnums> &CxpConnectionTestMode;

  /**
   * Description: Reports the current connection error count for test packets
   * recieved by the device on the connection selected by CxpConnectionSelector.
   * Visibility: Expert
   */
  GenApi::IInteger &CxpConnectionTestErrorCount;

  /**
   * Description: Reports the current count for test packets recieved by the
   * device on the connection selected by CxpConnectionSelector. Visibility:
   * Expert
   */
  GenApi::IInteger &CxpConnectionTestPacketCount;

  /**
   * Description: Activate automatic control of the Power over CoaXPress (PoCXP)
   * for the Link. Visibility: Expert
   */
  ...
  /**
   * Description: Reset the Power over CoaXPress (PoCXP) Link after an
   * over-current trip on the Device connection(s). Visibility: Expert
   */
