...
#define FLIR_SPINNAKER_VERSION_MINOR 25
#define FLIR_SPINNAKER_VERSION_TYPE 0
#define FLIR_SPINNAKER_VERSION_BUILD 52
...
/**
 * @brief The system object is used to retrieve the list of interfaces and
 * cameras available.
 */
...
public:
   ...
   *
   * @param updateInterface Determines whether or not UpdateInterfaceList() is
   ...
   *
   * @param updateInterfaces Determines whether or not updateInterfaceList() is
   * called before getting cameras from available interfaces on the system
   *
   * @param updateCameras Determines whether or not UpdateCameras() is called
   ...
   * internally calls UpdateCameras() for each interface it enumerates.  If the
   * list changed between this call and the last time UpdateCameras was called
   ...
   * @param updateInterface Determines whether or not UpdateInterfaceList() is
   * called before registering event for available interfaces on the system
   */
  virtual void RegisterInterfaceEvent(Event &evtToRegister,
                                      bool updateInterface = true);

  /**
   * Unregisters events for all available interfaces that are found on the
   * system
   *
   * @param evtToUnregister The event to unregister from the available
   * interfaces
   */
   ...
   *	- Error		- failures that are non-recoverable without user
   *intervention.
   *	- Warning	- failures that are recoverable without user
   *intervention.
   *	- Notice	- information about events such as camera arrival and
   *removal, initialization and deinitialization, starting and stopping image
   *acquisition, and feature modification.
   *	- Info		- information about recurring events that are generated
   *regularly such as information on individual images.
   *	- Debug		- information that can be used to troubleshoot the
   *system.
   *
   *
   ...
   * @param actionTime (Optional) Time when to assert a future action. Zero
   * means immediate action.
   * @param pResultSize (Optional) The number of results in the results array.
   * The value passed should be equal to the expected number of devices that
   * acknowledge the command. Returns the number of received results. If this
   * parameter is 0 or NULL, the function will return as soon as the command has
   * been broadcasted.
   * @param results (Optional) An Array with *pResultSize elements to hold the
   * action command result status. The buffer is filled starting from index 0.
   * If received results are less than expected number of devices that
   * acknowledge the command, remaining results are not changed. If received
   * results are more than expected number of devices that acknowledge the
   * command, extra results are ignored and not appended to array. This
   * parameter is ignored if pResultSize is 0. Thus this parameter can be NULL
   * if pResultSize is 0 or NULL.
   *
   */
  ...
  /**
   * Gets a reference to the system node map. The system must be initialized by
   * a call to System::GetInstance() first before a node map reference can be
