...
public:
   ...
   *
   */
  std::streamsize write(const char *buf, int64_t offs, int64_t len,
   ...
   *
   */
  std::streamsize read(char *buf, int64_t offs, std::streamsize len,
                       const char *pFileName);
...
public:
  ...
  bool is_open() const { return m_pAdapter != 0; }
...
private:
  ...
  int buffer_in() {
    ...
    std::streamsize retval =
        m_pAdapter->read(m_pBuffer, m_fpos, m_BufSize, m_file.c_str());
...
public:
  ...
  filebuf_type *open(Spinnaker::GenApi::INodeMap *pInterface,
    ...
    std::basic_streambuf<CharType, Traits>::setp(m_pBuffer,
                                                 m_pBuffer + bufSize);
...
protected:
    ...
    if (n < epptr() - pptr()) {
      ...
      for (std::streamsize i = 0; i < n; ++i) {
        if (traits_type::eq_int_type(
                std::basic_streambuf<CharType, Traits>::sputc(s[i]),
                traits_type::eof()))
          return i;
      }
      return n;
  ...
  int_type overflow(int_type c = traits_type::eof()) {
    if (buffer_out() < 0) {
      ...
      if (!traits_type::eq_int_type(c, traits_type::eof()))
        return std::basic_streambuf<CharType, Traits>::sputc(
            static_cast<char_type>(c));
      else
        return traits_type::not_eof(c);
    }
  }
  int sync() { return static_cast<int>(buffer_out()); }
...
public:
  ...
  ODevFileStreamBase(Spinnaker::GenApi::INodeMap *pInterface,
                     const char *pFileName,
  ...
  ODevFileStreamBase(INodeMap *pInterface, const char *pFileName,
                     std::ios_base::openmode mode = std::ios_base::out |
                                                    std::ios_base::trunc)
...
#error Unknown C++ library
   ...
   *
   * @param  pInterface  NodeMap of the device to which the FileProtocolAdapter
   * is attached
   * @param  pFileName  Name of the file to open
   * @param  mode open mode
   */

  void open(INodeMap *pInterface, const char *pFileName,
            std::ios_base::openmode mode = std::ios_base::out |
                                           std::ios_base::trunc) {
...
public:
  ...
  IDevFileStreamBase(Spinnaker::GenApi::INodeMap *pInterface,
                     const char *pFileName,
                     std::ios_base::openmode mode = std::ios_base::in)
  ...
  IDevFileStreamBase(INodeMap *pInterface, const char *pFileName,
                     std::ios_base::openmode mode = std::ios_base::in)
...
#error Unknown C++ library
   ...
   * @param  mode open mode
   */
  void open(Spinnaker::GenApi::INodeMap *pInterface, const char *pFileName,
            std::ios_base::openmode mode = std::ios_base::in) {
    if (!m_streambuf.open(pInterface, pFileName, mode))
      this->setstate(std::ios_base::failbit);
    else
