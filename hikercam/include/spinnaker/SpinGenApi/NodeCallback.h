...
typedef enum _ECallbackType {
  cbPostInsideLock = 1, /*!> callback is fired on leaving the tree inside the
                           lock-guarded area*/
  cbPostOutsideLock = 2 /*!> callback is fired on leaving the tree inside the
                           lock-guarded area*/
} ECallbackType;
...
public:
  CNodeCallback(INode *pNode, ECallbackType CallbackType)
      : m_pNode(pNode), m_CallbackType(CallbackType) {}
   ...
   */
  INode *GetNode() { return m_pNode; }

  ECallbackType GetCallbackType() { return m_CallbackType; }
...
public:
   ...
   * Constructor
   */
  Function_NodeCallback(INode *pNode, const Function &function,
...
CNodeCallback *make_NodeCallback(INode *pNode, Function function,
                                 ECallbackType CallbackType) {
  return static_cast<CNodeCallback *>(
      new Function_NodeCallback<Function>(pNode, function, CallbackType));
}
...
public:
   ...
   * Constructor
   */
  Member_NodeCallback(INode *pNode, Client &client, Member member,
                      ECallbackType CallbackType)
      : CNodeCallback(pNode, CallbackType), m_Client(client),
 ...
 */
template <class Client, class Member>
CNodeCallback *make_NodeCallback(INode *pNode, Client &client, Member member,
                                 ECallbackType CallbackType) {
  return static_cast<CNodeCallback *>(new Member_NodeCallback<Client, Member>(
      pNode, client, member, CallbackType));
}
