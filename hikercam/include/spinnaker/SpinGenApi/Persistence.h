...
public:
   ...
   * @param pErrorList If an error occurs during loading the error message is
   * stored in the list and the loading continues
   */
  /*! For Verify=true the list of names in the feature bag is replayed again.
      If a node is a selector it's value is set to the value from the feature
     bag If not the value is read from the camera and compared with the value
     from the feature bag.
  */
  bool LoadFromBag(INodeMap *pNodeMap, bool Verify = true,
   ...
   *   @param MaxNumPersistSkriptEntries The max number of entries in the
 ...
 */
// note: this method must be inlined because it uses istream in the parameter
// list
 ...
 */
// note: this method must be inlined because it uses ostream in the parameter
// list
