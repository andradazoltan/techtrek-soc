...
/**
 * This verifies at runtime if there was no loss of data if an int64_t was
 * downcast to type T (e.g. int32_t)
 */
 ...
 */
SPINNAKER_API bool
DoesEnvironmentVariableExist(const Spinnaker::GenICam::gcstring &VariableName);
...
GetValueOfEnvironmentVariable(const gcstring &VariableName);
 ...
 * The path to the cache folder can be stored by calling
 * SetGenICamCacheFolder(). If GetGenICamCacheFolder() is called before
 * SetGenICamCacheFolder(), it will return the value of environment variable
 * GENICAM_CACHE_Vx_y.  If this environment variable does not exist, an
 ...
 * SetGenICamLogConfig(), it will return the value of environment variable
 * GENICAM_LOG_CONFIG_Vx_y.  If this environment variable does not exist, an
 * exception will be thrown.
 */
 ...
 * The path to the CLProtocol folder can be stored by calling
 * SetGenICamCLProtocolFolder(). If GetGenICamCLProtocolFolder() is called
 * before SetGenICamCLProtocolFolder(), it will return the value of environment
 * variable GENICAM_CLPROTOCOL.  If this environment variable does not exist, an
 * exception will be thrown.
 */
 ...
 */
SPINNAKER_API void
Tokenize(const gcstring &str,     /*!< string to be split*/
 ...
 */
SPINNAKER_API void
GetFiles(const gcstring &FileTemplate, /*!> The file template. Can contain
                                          environment variables.*/
         gcstring_vector
             &FileNames, /*!> A list of files matching the file template*/
         const bool DirectoriesOnly =
             false); /*!> true = only subdirectories (ex . and ..) are
                        retrieved; false = only files are retrieved*/

/**
 * Gets the full path to the module (DLL/SO) containing the given \a pFunction;
