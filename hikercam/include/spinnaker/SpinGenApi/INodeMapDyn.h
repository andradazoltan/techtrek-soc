...
interface SPINNAKER_API_ABSTRACT INodeMapDyn : virtual public INodeMap {
  ...
  virtual void LoadXMLFromFileInject(
      ...
      const GenICam::gcstring &InjectFileName) = 0;
  ...
  virtual void LoadXMLFromStringInject(
      ...
      const GenICam::gcstring &InjectXMLData) = 0;
  ...
  virtual void PreprocessXMLFromFile(
      const GenICam::gcstring
          &XMLFileName, /*!> The name of the XML file to process*/
      const GenICam::gcstring
          &StyleSheetFileName, /*!> Optional name of a style sheet which is
                                  applied after the pre-processor (can be empty
                                  string)*/
      /*!> This has no effect if the OutputFileName is an empty string*/
      const GenICam::gcstring
          &OutputFileName, /*!> Optional name of an output file into which the
                              processed data is written (can be empty string)*/
      const uint32_t XMLValidation =
          xvDefault /*!> Optional bit mask formed from EXMLValidation
                       enumeration indicating which tests should be performed on
                       the XML file*/
      ) = 0;
  ...
  virtual void MergeXMLFiles(
      const GenICam::gcstring
          &TargetFileName, /*!> Name of the target XML file to process*/
      const GenICam::gcstring
          &InjectedFileName, /*!> Name of the Injected XML file to process*/
  ...
  virtual void ExtractIndependentSubtree(
      const GenICam::gcstring
          &XMLData, /*!> The XML data the subtree is extracted from.*/
      const GenICam::gcstring
          &InjectXMLData, /*!> Optional XML data that is injected before
                             extracting the subtree. No effect if an empty
                             string is passed.*/
      const GenICam::gcstring &
          SubTreeRootNodeName, /*!> The name of the node that represents the
                                  root of the subtree that shall be extracted.*/
      GenICam::gcstring
  ...
  /*! Each list entry is a string with the format "<Major>.<Minor>" were <Major>
     ...
     v1.1 and v1.2 are supported. The SubMinor version number is not given since
     it is for fully compatible bug fixes only
  */
  virtual void GetSupportedSchemaVersions(GenICam::gcstring_vector &
                                          SchemaVersions) = 0;
  ...
  /**
   * Loads a Zipped XML, checks it for correctness, pre-processes it, caches it,
   * and optionally applies a style sheet, and optionally writes it to a file
   */
