...
public:
  ...
  virtual gcstring substr(size_t offset = 0,
                          size_t count = GCSTRING_NPOS) const;
...
private:
  // redundant pointer to make the possible to see the contents of the string in
  // the debugger
  const char *m_psz;
 ...
 * STL getline
 */
inline std::istream &getline(std::istream &is,
 ...
 */
inline std::istream &getline(std::istream &is,
                             Spinnaker::GenICam::gcstring &str, char delim) {
  try {
    std::string tmp;
