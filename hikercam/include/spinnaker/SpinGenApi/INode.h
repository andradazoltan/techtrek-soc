 ...
 */
interface SPINNAKER_API_ABSTRACT INode : virtual public ISelector,
  ...
  virtual void GetChildren(GenApi::NodeList_t & Children,
                           ELinkType LinkType = ctReadingChildren) const = 0;
   ...
   */
  virtual void GetPropertyNames(GenICam::gcstring_vector & PropertyNames)
      const = 0;

  /**
   * Retrieves a property plus an additional attribute by name
   * If a property has multiple values/attribute they come with Tabs as
   * delimiters
   */
  ...
  /**
   * Retrieves the a node which describes the same feature so that it can be
   * casted
   */
  ...
  /**
   * True if the node can be reached via category nodes from a category node
   * named "Root"
   */
 ...
 * Checks if a node is readable
 */
inline bool IsReadable(const IBase &r) { return IsReadable(r.GetAccessMode()); }
 ...
 * Checks if a node is writable
 */
inline bool IsWritable(const IBase &r) { return IsWritable(r.GetAccessMode()); }

/**
 * Tests if implemented
 */
inline bool IsImplemented(EAccessMode AccessMode) { return AccessMode != NI; }
 ...
 * Checks if a node is implemented
 */
inline bool IsImplemented(const IBase &r) { return IsImplemented(&r); }
