 ...
 * @param TCameraParams  The camera specific parameter class (auto generated
 * from camera xml file)
 */
class SPINNAKER_API NodeMap : public virtual INodeMap,
...
public:
  ...
  //! note Can only be used if the class TCameraParams was auto generated from a
  specific camera xml file void LoadDLL(void);
  */

  /*!
  //! Creates the object from a DLL whose name is deduced from vendor and model
  name void LoadDLL(GenICam::gcstring VendorName, GenICam::gcstring ModelName);
  */
  ...
  */
  ...
  /**
   * Creates the object from a XML target and an inject file with given file
   * name
   */
  ...
  /*!
  //! Loads an XML, checks it for correctness, applies a style-sheet and outputs
  it void PreprocessXMLFromFile(const GenICam::gcstring& XMLFileName, const
  GenICam::gcstring& StyleSheetFileName, const GenICam::gcstring&
  OutputFileName, const uint32_t XMLValidation = xvDefault);

  //! Loads a Zipped XML, checks it for correctness, applies a style-sheet and
  outputs it void PreprocessXMLFromZIPFile(const GenICam::gcstring& ZIPFileName,
  ...
  file to process const GenICam::gcstring& InjectedFileName,    *< Name of the
  Injected XML file to process const GenICam::gcstring& OutputFileName       *<
  Name of the output file
      );

  //! Extract independent subtree
  virtual void ExtractIndependentSubtree(
      const GenICam::gcstring& XMLData,            *< The XML data the subtree
  is extracted from. const GenICam::gcstring& InjectXMLData,      *< Optional
  XML data that is injected before extracting the subtree. No effect if an empty
  string is passed. const GenICam::gcstring& SubTreeRootNodeName,*< The name of
  the node that represents the root of the subtree that shall be extracted.
      GenICam::gcstring& ExtractedSubtree          *< The returned extracted
  subtree as string.
      );
  */
  ...
  */
  virtual void
  GetSupportedSchemaVersions(GenICam::gcstring_vector &SchemaVersions);
...
private:
  ...
  void
  PopulateNodeMap(void *pNodeMap,
                  std::map<GenICam::gcstring, std::shared_ptr<INode>> &outMap);
