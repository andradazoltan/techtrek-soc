...
/**
 * @brief Encapsulates a GenApi pointer dealing with the dynamic_cast
 * automatically
 */
...
public:
   ...
   */
  void operator=(B *pB) { m_pT = dynamic_cast<T *>(pB); }
  /**
   * Dereferencing
   */
  operator T *(void)const {
    if (nullptr == m_pT) {
      throw Spinnaker::Exception(
          __LINE__, __FILE__, __FUNCTION__, __DATE__, __TIME__,
   ...
   */
  bool IsValid() const throw() { return m_pT != nullptr; }

  /**
   * true if the pointer is valid
   */
  operator bool(void) const throw() { return m_pT != nullptr; }
   ...
   * pointer equal
   */
  bool operator==(const CPointer<T, B> &rT) const { return m_pT == rT.m_pT; }
  ...
  bool operator==(int nMustBeNull) const {
    if (0 != nMustBeNull) {
      throw Spinnaker::Exception(
          __LINE__, __FILE__, __FUNCTION__, __DATE__, __TIME__,
          "LogicalErrorException argument must be NULL", GENICAM_ERR_LOGICAL);
    }
    return NULL == m_pT;
  }
...
public:
   ...
   */
  void operator=(IBase *pB) { CPointer<IFloat, IBase>::operator=(pB); }
