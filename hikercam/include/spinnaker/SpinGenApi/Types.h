...
/**
 * typedef describing the different validity checks which can be performed on an
 * XML file
 */
/*!
...
typedef enum _EXMLValidation {
  ...
  xvCycles =
      0x00000002L, /** checks for write and dependency cycles (implies xvLoad)*/
  xvSFNC = 0x00000004L,    /** checks for conformance with the standard feature
                              naming convention (SFNC)*/
...
typedef enum _EDisplayNotation {
  fnAutomatic,  /** the notation if either scientific or fixed depending on what
                   is shorter*/
...
typedef enum _ELinkType {
  ctParentNodes, /** All nodes for which this node is at least an invalidating
                    child*/
  ctReadingChildren, /** All nodes which can be read from*/
  ctWritingChildren, /** All nodes which can write a value further down the node
                        stack*/
  ctInvalidatingChildren, /** All directly connected nodes which invalidate this
                             node*/
  ctDependingNodes, /** All directly or indirectly connected nodes which are
                       invalidated by this nodes (i.e. which are dependent on
                       this node)*/
...
typedef enum _EInputDirection {
  idFrom, /** Indicates a swiss knife that it is used as worker for a converter
             computing FROM*/
  idTo,   /** Indicates a swiss knife that it is used as worker for a converter
             computing TO*/
