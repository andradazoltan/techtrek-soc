...
public:
   ...
   */
  virtual EAccessMode GetAccessMode() const { return RW; }

  /**
   * Get the type of the main interface of a node
   */
  virtual EInterfaceType GetPrincipalInterfaceType() const { return intfIPort; }
  ...
  virtual void Read(void *pBuffer, int64_t Address, int64_t Length) {
    ...
    if (InternalAddress < 0 || Length < 0 ||
        InternalAddress + Length > sizeof(CDataStruct)) {
      // throw RUNTIME_EXCEPTION("CTestPortStruct::Read - Invalid address and/or
      // length");
    }
  ...
  virtual void Write(const void *pBuffer, int64_t Address, int64_t Length) {
    ...
    if (InternalAddress < 0 || Length < 0 ||
        InternalAddress + Length > sizeof(CDataStruct)) {
      // throw RUNTIME_EXCEPTION("CTestPortStruct::Write - Invalid address
      // and/or length");
    }
   ...
   */
  int64_t GetNumReads() { return m_NumReads; }

  /**
   * Returns the number of writes since lastReset Statistics
   */
  int64_t GetNumWrites() { return m_NumWrites; }
