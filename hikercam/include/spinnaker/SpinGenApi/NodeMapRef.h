...
#else
/**
 * makes sure the dynamic_cast operator is implemented in the DLL (due to a
 * Linux bug)
 */
...
public:
  ...
  */
  virtual void
  _GetSupportedSchemaVersions(GenICam::gcstring_vector &SchemaVersions);
...
private:
  ...
  int *_pRefCount;
  void Release();
  void Attach(INodeMap *pNodeMap, const GenICam::gcstring &DeviceName,
              int *pRefCount);
};
...
template <class TCameraParams>
inline CNodeMapRefT<TCameraParams>::CNodeMapRefT(
    INodeMap *pNodeMap, const GenICam::gcstring &DeviceName)
    : _Ptr(NULL), _DeviceName(DeviceName), _pRefCount(NULL) {
  assert(pNodeMap);
...
void GenApi::CNodeMapRefT<TCameraParams>::Attach(
    INodeMap *pNodeMap, const GenICam::gcstring &DeviceName, int *pRefCount) {
  // Must be empty
  assert(_Ptr == NULL);
...
template <class TCameraParams>
CNodeMapRefT<TCameraParams> &GenApi::CNodeMapRefT<TCameraParams>::
operator=(INodeMap *pNodeMap) {
  Release();
  assert(pNodeMap);
  if (pNodeMap) {
...
template <class TCameraParams>
CNodeMapRefT<TCameraParams> &GenApi::CNodeMapRefT<TCameraParams>::
operator=(const CNodeMapRefT &Them) {
  Release();
  Attach(Them._Ptr, Them._DeviceName, Them._pRefCount);
  return *this;
}
...
template <class TCameraParams>
inline void _LoadXMLFromFileInject(const GenICam::gcstring &TargetFileName,
                                   const GenICam::gcstring &InjectFileName);
...
template <class TCameraParams>
inline void _LoadXMLFromStringInject(const GenICam::gcstring &TargetXMLData,
                                     const GenICam::gcstring &InjectXMLData);
...
/**
 * @brief Empty base class used by class CNodeMapRef as generic template
 * argument
 */
 ...
 *
 * @note This class is a simple typedef definition. The class syntax is only
 * used, because Doxygen has to generate a useful documentation.
 */
