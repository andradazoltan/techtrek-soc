...
public:
  ...
  int64_t GetRecursionCount();
    ...
    (defined(__linux__) || defined(__APPLE__) || defined(VXWORKS))
...
public:
  ...
  /// Get a new lock
  Lock GetLock() const { return Lock(*this); }
};
...
protected:
  ...
  sem_t *m_handle;
#elif VXWORKS
  // There are no named locks on VxWorks. While we could use a single global
  // lock, we will just rely on the caller to add their own locking in
#else
#error No/unknown platform thread support
#endif
 ...
 * We can't do automatic locking here since there is no returnvalue for
 * contructors
 */
//-----------------------------------------------------------------
// unlocks the global lock object on destruction
// this is for automatic UNLOCKING only.
// we can't do automatic locking here since we don't get a returnvalue from the
// c'tor
//-----------------------------------------------------------------
...
public:
  CGlobalLockUnlocker(CGlobalLock &lock)
      : m_Lock(lock), m_enabled(true) // this allows the auto unlock to be
                                      // turned off for messy code structures
  {
    // explicitly don't lock the object here since we want to do this manually
    // and handle the return value properly
  }

  ~CGlobalLockUnlocker() { UnlockEarly(); }

  /**
   * This function allows to unlock the object early before the object is
   * destroyed
   */
