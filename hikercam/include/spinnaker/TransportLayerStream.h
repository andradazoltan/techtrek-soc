...
public:
  ...
  /**
   * Description: Counts the number of image buffers that arrived since stream
   * started. Visibility: Expert
   */
  GenApi::IInteger &StreamTotalBufferCount;

  /**
   * Description: DEPRECATED; Replaced by StreamBufferCountManual. Controls the
   * number of buffers to be used on this stream upon acquisition start when in
   * manual mode. Visibility: Invisible
   */
  GenApi::IInteger &StreamDefaultBufferCount;

  /**
   * Description: DEPRECATED; Replaced by StreamBufferCountMax. Controls the
   * maximum number of buffers that should be used on this stream. This value is
   * calculated based on the available system memory. Visibility: Invisible
   */
  GenApi::IInteger &StreamDefaultBufferCountMax;

  /**
   * Description: DEPRECATED; Replaced by StreamBufferCountMode. Controls access
   * to setting the number of buffers used for the stream. Locked to Manual mode
   * on 32-bit Windows due to memory constraints. Visibility: Invisible
   */
  GenApi::IEnumerationT<StreamDefaultBufferCountModeEnum>
      &StreamDefaultBufferCountMode;

  /**
   * Description: Controls the number of buffers to be used on this stream upon
   * acquisition start when in manual mode. Visibility: Expert
   */
  GenApi::IInteger &StreamBufferCountManual;

  /**
   * Description: Displays the number of buffers to be used on this stream upon
   * acquisition start. Recalculated on acquisition start if in auto mode.
   * Visibility: Expert
   */
  GenApi::IInteger &StreamBufferCountResult;

  /**
   * Description: Controls the maximum number of buffers that should be used on
   * this stream. This value is calculated based on the available system memory.
   * Visibility: Expert
   */
  GenApi::IInteger &StreamBufferCountMax;

  /**
   * Description: Controls access to setting the number of buffers used for the
   * stream. Locked to Manual mode on 32-bit Windows due to memory constraints.
   * Visibility: Expert
   */
  ...
  /**
   * Description: Maximum number of resend requests per image. Each resend
   * request consists of a span of consecutive packet IDs. Visibility: Expert
   */
  GenApi::IInteger &GevMaximumNumberResendRequests;

  /**
   * Description: Time in milliseconds to wait after the image trailer is
   * received and before the image is completed by the driver. Visibility:
   * Expert
   */
  ...
  /**
   * Description: Displays number of packets received after retransmit request
   * on this stream. Visibility: Expert
   */
  GenApi::IInteger &GevResendPacketCount;

  /**
   * Description: Displays number of incomplete images with missing
   * leader/trailer information. Visibility: Expert
   */
  GenApi::IInteger &StreamFailedBufferCount;

  /**
   * Description: Displays number of dropped images caused by driver running out
   * of buffers. Visibility: Expert
   */
  GenApi::IInteger &StreamBufferUnderrunCount;

  /**
   * Description: Displays number of packets requested to be retransmitted on
   * this stream. Visibility: Expert
   */
  GenApi::IInteger &GevResendRequestCount;

  /**
   * Description: Controls the image breakup size that should be used on this
